...
export const getStaffList = async (params = {}) => {
  const response = await api.get("/v1/staffing/staff", { params });
...
export const createStaff = async (data) => {
  const response = await api.post("/v1/staffing/staff", data);
  return response.data?.data;
};

// 6. Update Staff
export const updateStaff = async (id, data) => {
  const response = await api.put(`/v1/staffing/staff/${id}`, data);
  return response.data?.data;
};
...
export const getStaffingDashboard = async () => {
  const response = await api.get("/v1/staffing/dashboard");
  return response.data?.data;
};

// (Hàm phụ) Lấy danh sách phân công (để hiển thị bảng Race Assignment)
export const getRaceAssignments = async (params = {}) => {
  const response = await api.get("/v1/staffing/assignments", { params });
  return response.data?.data?.content || response.data?.data;
};

// 7. Assign Referee (POST)
export const assignReferee = async (data) => {
  const response = await api.post("/v1/staffing/assignments", data);
  return response.data?.data;
};
...
export const reassignReferee = async (id, data) => {
  const response = await api.put(`/v1/staffing/assignments/${id}`, data);
  return response.data?.data;
};

// 9. Remove Assignment (DELETE)
export const removeAssignment = async (id) => {
  const response = await api.delete(`/v1/staffing/assignments/${id}`);
