...
/** Brand mark — green icon + bold wordmark — reused from the app header styling. */
function Brand({ onClick }: { onClick?: () => void }) {
  return (
    <Link to={SPECTATOR_HOME} onClick={onClick} className="flex items-center gap-2">
...
/** Avatar + sign-out dropdown — reuses the shared Avatar and `logout` from auth. */
...
/**
 * Top-navigation shell for the SPECTATOR area (Figma "Equine Elite" spectator design).
 * Mirrors DashboardLayout's auth/<Outlet/> wiring but renders a sticky top nav instead of a sidebar.
 * ProtectedRoute + RoleRoute guard this layout from the router.
 */
