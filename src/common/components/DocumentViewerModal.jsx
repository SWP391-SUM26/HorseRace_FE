 ...
 *
 * `path` is the auth-gated API path (relative to the apiClient baseURL) to fetch the file
 * bytes from.
 */
export function DocumentViewerModal({ open, onClose, path, fileName }) {
      ...
      {loading ? (
        ...
        <div className="flex h-64 flex-col items-center justify-center gap-2 text-center">
          ...
          <p className="text-xs text-muted">
            It may be restricted, unavailable, or you may need to sign in again.
          </p>
        </div>
      ) : objectUrl ? (
        mimeType.startsWith("image/") ? (
          ...
          <iframe
            src={objectUrl}
            title={title}
            className="h-[75vh] max-h-[75vh] w-full rounded-lg border border-border"
          />
        ) : (
