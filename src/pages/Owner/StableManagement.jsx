...
const UPCOMING_EVENTS = [
  { id: 'ev_001', name: "Ascot Summer Stakes", track: "Ascot Racecourse", date: "Jul 14, 2026" },
  { id: 'ev_002', name: "The Gold Jubilee", track: "Epsom Downs", date: "Jul 22, 2026" },
  { id: 'ev_003', name: "Champions Cup", track: "York Racecourse", date: "Aug 05, 2026" },
  { id: 'ev_004', name: "Belmont Stakes", track: "Belmont Park", date: "Oct 20, 2026" }
];
...
export default function StableManagement() {
  ...
  const handleDeleteHorse = async (id, name, e) => {
    e.stopPropagation();
    if (window.confirm(`Are you sure you want to retire/delete ${name} from your stable?`)) {
    ...
    <div className={styles.stableContainer}>
      ...
      <div className={styles.pageHeader}>
        <div>
          ...
          <p className={styles.pageSubtitle}>Welcome back, Mr. Sterling. Your stable is performing well.</p>
        </div>
      ...
      <div className={styles.statsRow}>
        ...
        <div className={styles.statCard}>
          ...
          <div className={styles.statsWrapper}>
            <div className={styles.statBox}>
              ...
              <strong className={styles.statNumber}>{loading ? '...' : totalHorses}</strong>
            </div>
            <div className={styles.statDivider} />
            <div className={styles.statBox}>
              <span className={styles.statLabel}>FIT FOR RACING</span>
              <strong className={styles.statNumber} style={{ color: '#16a34a' }}>{loading ? '...' : fitCount}</strong>
            </div>
            <div className={styles.statDivider} />
            <div className={styles.statBox}>
              <span className={styles.statLabel}>WIN RATE</span>
              <strong className={styles.statNumber} style={{ color: '#d97706' }}>{stableOverview.winRate}</strong>
            </div>
          </div>
          <div className={styles.cardFooter}>
            <span>{loading ? '...' : restingCount} Horses currently unfit/resting</span>
            <a href="#medical" className={styles.footerLink}>View Medical Reports</a>
          </div>
        </div>
        ...
        <div className={styles.earningsCard}>
          ...
          <div className={styles.earningsTrendWrap}>
            ...
            <span className={styles.trendPercent}>↑{stableOverview.ytdEarningsTrend}</span>
          </div>
          ...
          <div className={styles.rosterCard}>
            ...
            <table className={styles.rosterTable}>
              ...
              <tbody>
                {loading ? (
                  <tr>
                    <td colSpan="4" className={styles.emptyTable}>Loading stable data...</td>
                  </tr>
                    ...
                    >
                      <td>
                        <div className={styles.horseCell}>
                          <img src={horse.image} alt={horse.name} className={styles.horseThumb} />
                          <div>
                            <span className={styles.horseName}>{horse.name}</span>
                            <span className={styles.horseDesc}>{horse.details}</span>
                          </div>
                        </div>
                      </td>
                      <td>
                        <span className={`${styles.statusBadge} ${horse.status === 'FIT TO RACE' ? styles.statusFit : styles.statusResting}`}>
                          {horse.status === 'FIT TO RACE' ? 'FIT TO RACE' : 'RESTING'}
                        </span>
                      </td>
                      <td>
                        <div className={styles.nextRaceCell}>
                          <strong className={styles.nextRaceDate}>{horse.nextRace}</strong>
                          {horse.track && <span className={styles.nextRaceTrack}>{horse.track}</span>}
                        </div>
                      </td>
                      <td style={{ textAlign: 'right' }} onClick={(e) => e.stopPropagation()}>
                        <div className={styles.actionsWrap}>
                          ...
                          <button 
                            className={styles.actionBtnIcon} 
                            onClick={(e) => handleDeleteHorse(horse.id, horse.name, e)}
                            style={{ color: '#ef4444' }}
                            title="Retire Horse"
                          >
                            🗑️
                  ...
                  <tr>
                    <td colSpan="4" className={styles.emptyTable}>No horses found matching current criteria.</td>
                  </tr>
                )}
              </tbody>
            </table>
        ...
        <div className={styles.sidebarSection}>
            ...
            <div className={styles.widgetBody}>
              {stableOverview.jockeyInvites.map(invite => (
                <div key={invite.id} className={styles.inviteItem}>
                  ...
                  <p className={styles.inviteText}>Invited {invite.jockeyName} for {invite.race}</p>
                  <button 
                    className={styles.resendBtn}
                    onClick={() => triggerToast(`Resent invitation to ${invite.jockeyName} for ${invite.race}`)}
                  >
                    Resend
                  </button>
                </div>
          ...
          <div className={styles.widgetCard}>
              ...
              {stableOverview.healthDocs.map(doc => {
                ...
                return (
                  <div key={doc.id} className={styles.docItem}>
                    <span className={isWarning ? styles.docIconWarning : styles.docIconSuccess}>
                      {isWarning ? '⚠️' : '✅'}
                    </span>
                    <div className={styles.docInfo}>
                      <span className={styles.docHorse}>{doc.horseName}</span>
                      <span className={isWarning ? styles.docStatusWarning : styles.docStatusSuccess}>
            ...
            <div className={styles.widgetFooter}>
              <a href="#documents" className={styles.widgetFooterLink}>Manage Documents</a>
            </div>
          </div>
        </div>
      </div>
        ...
        <div className={styles.modalOverlay}>
          <div className={styles.modalContent}>
            <div className={styles.modalHeader}>
              ...
              <button className={styles.modalCloseBtn} onClick={() => setIsCreateOpen(false)}>✕</button>
            </div>
            <form onSubmit={handleCreateHorse} className={styles.modalForm}>
              ...
              <div className={styles.formGroup}>
                <label className={styles.formLabel}>INITIAL READINESS STATUS</label>
                <select 
                  value={formStatus} 
              ...
              <div className={styles.modalActions}>
                <button type="button" className={styles.btnGhost} onClick={() => setIsCreateOpen(false)}>
                  Cancel
                </button>
        ...
        <div className={styles.modalOverlay}>
          <div className={styles.modalContent}>
            <div className={styles.modalHeader}>
              ...
              <button className={styles.modalCloseBtn} onClick={() => setIsEditOpen(false)}>✕</button>
            </div>
        ...
        <div className={styles.modalOverlay} onClick={() => setIsDetailOpen(false)}>
          <div className={styles.drawerContent} onClick={(e) => e.stopPropagation()}>
            <div className={styles.drawerHeader}>
              ...
              <button className={styles.drawerCloseBtn} onClick={() => setIsDetailOpen(false)}>✕</button>
            </div>
            
            <div className={styles.drawerBody}>
              {/* Image & Basic Details */}
              <div className={styles.detailHero}>
                <img src={selectedHorse.image} alt={selectedHorse.name} className={styles.detailLargeImage} />
                <h2 className={styles.detailName}>{selectedHorse.name}</h2>
                <div className={styles.detailPills}>
                  <span className={styles.detailPill}>{selectedHorse.age}</span>
                  <span className={styles.detailPill}>{selectedHorse.breed}</span>
                  <span className={`${styles.statusBadge} ${selectedHorse.status === 'FIT TO RACE' ? styles.statusFit : styles.statusResting}`}>
              ...
              <div className={styles.drawerInlineActions}>
                <button 
                  ...
                  onClick={(e) => { setIsDetailOpen(false); handleOpenEdit(selectedHorse, e); }}
                  style={{ flex: 1 }}
                >
                  ✏️ Edit Profile
                </button>
                <button 
                  className={styles.btnGhost} 
                  onClick={(e) => handleDeleteHorse(selectedHorse.id, selectedHorse.name, e)}
              ...
              <div className={styles.detailSection}>
                <div className={styles.sectionHeader}>
                  ...
                  <h4 className={styles.sectionTitle}>Medical Records & Status</h4>
                </div>
                <div className={styles.medicalBox}>
                  ...
                  <div className={styles.medicalField}>
                    ...
                    <strong style={{ color: selectedHorse.cogginsTest === 'Up to date' ? '#16a34a' : '#ef4444' }}>
                      {selectedHorse.cogginsTest}
                    </strong>
                  </div>
                  <div className={styles.medicalField}>
                    <span>VACCINATIONS:</span>
                    <strong style={{ color: '#16a34a' }}>{selectedHorse.vaccines}</strong>
                  </div>
                  ...
                  >
                    Toggle Fitness Status (Mark as {selectedHorse.status === 'FIT TO RACE' ? 'RESTING' : 'FIT FOR RACING'})
                  </button>
                </div>
              </div>
              ...
              <div className={styles.detailSection}>
                <div className={styles.sectionHeader}>
                  <span className={styles.sectionTitleIcon}>🏁</span>
                  <h4 className={styles.sectionTitle}>Assign Horse to Upcoming Race</h4>
                </div>
                <form onSubmit={handleAssignRace} className={styles.assignForm}>
                  ...
                  <button type="submit" className={styles.btnPrimary} style={{ width: '100%', marginTop: '10px' }}>
              ...
              <div className={styles.detailSection}>
                ...
                <div className={styles.historyList}>
                  {selectedHorse.raceHistory && selectedHorse.raceHistory.length > 0 ? (
                    selectedHorse.raceHistory.map((hist, idx) => (
                      <div key={idx} className={styles.historyRow}>
                        <div className={styles.historyMeta}>
                          <strong>{hist.race}</strong>
                          <span>{hist.date} • Jockey: {hist.jockey}</span>
                        </div>
                        <div className={styles.historyStats}>
                          <span className={styles.historyPlace}>{hist.place}</span>
                          <span className={styles.historyPrize}>{hist.prize}</span>
                        </div>
                      </div>
                    ))
                  ) : (
                    <p className={styles.emptyHistory}>This horse has no recorded past race entries.</p>
                  )}
                </div>
              </div>
            </div>
