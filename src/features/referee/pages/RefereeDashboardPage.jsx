...
import { AlertTriangle, Check, Radio, Users, X } from "lucide-react";
...
function RefereeDashboardPage() {
  ...
  useEffect(() => {
    ...
    const id = setInterval(() => setRemaining((r) => r == null ? r : Math.max(0, r - 1)), 1e3);
  ...
  />
      ...
      {isPending ? <div className="grid gap-4 lg:grid-cols-3">
          <Skeleton className="h-40 w-full rounded-2xl lg:col-span-2" />
          <Skeleton className="h-40 w-full rounded-2xl" />
        </div> : isError || !data ? <EmptyState title="Could not load the dashboard" description="Please reload the page." /> : <>
            ...
            <div className="flex flex-col rounded-2xl border border-brand-700 bg-brand-800 p-6 text-white shadow-sm lg:col-span-2">
              ...
              {data.nextRace ? <>
                  <div className="mt-1 flex items-center gap-3">
                    <h2 className="text-2xl font-semibold">{data.nextRace.name}</h2>
                    <span className="rounded-md bg-white/15 px-2 py-0.5 text-xs font-semibold">
                      {data.nextRace.raceCode}
                    </span>
                  </div>
                  <p className="mt-4 text-xs uppercase tracking-wide text-white/60">Time to post</p>
                  <p className="text-4xl font-semibold tabular-nums">
                    {remaining != null && remaining > 0 ? fmtCountdown(remaining) : "\u2014"}
                  </p>
                  <p className="mt-2 text-xs text-white/60">
                    {remaining != null && remaining > 0 ? `Status: ${data.nextRace.status}` : "Post time has passed."}
                  </p>
  ...
  >
                      <Users size={16} /> View Runners
                    </Link>
                    <Link
    to="/app/referee/live"
    className="inline-flex h-10 items-center gap-2 rounded-lg border border-white/20 bg-white/10 px-4 text-sm font-medium text-white transition-colors hover:bg-white/20"
  >
                      <Radio size={16} /> Live Paddock Feed
                    </Link>
                  </div>
                </> : <p className="mt-2 text-white/70">No upcoming race scheduled.</p>}
            </div>
    ...
    /* Priority alerts */
            ...
            <Card>
              <CardBody className="flex flex-col gap-3">
                ...
                {data.alerts.length === 0 ? <p className="text-sm text-muted">No active alerts.</p> : <ul className="flex flex-col gap-2">
                    {data.alerts.map((a, i) => <li
  ...
  >
    ...
    /* Inspection summary KPIs */
          ...
          <div className="mt-6 grid gap-4 sm:grid-cols-4">
            <StatCard label="Cleared" value={data.inspectionSummary.cleared} hint="Inspections" />
            <StatCard label="Pending" value={data.inspectionSummary.pending} hint="Inspections" />
            <StatCard label="Vet Check" value={data.inspectionSummary.vetCheck} hint="Inspections" />
            <StatCard label="Total Runners" value={data.inspectionSummary.total} />
          </div>
          ...
          <div className="mt-6">
            <Card>
              <CardBody className="flex flex-col gap-3">
                ...
                {data.dutyRoster.length === 0 ? <p className="text-sm text-muted">No stewards assigned.</p> : <ul className="flex flex-col gap-2">
                    {data.dutyRoster.map((d) => <li
    key={d.refereeUserId}
    className="flex items-center justify-between gap-3 rounded-xl border border-border px-4 py-2.5"
  >
                        <span className="text-sm font-medium text-ink">{d.refereeName}</span>
                        <div className="flex items-center gap-2">
                          <Badge tone="info">{d.panelRole}</Badge>
                          <span className="text-xs text-muted">{d.station ?? "\u2014"}</span>
                        </div>
                      </li>)}
                  </ul>}
              </CardBody>
...
function InspectionMiniTable({ raceId }) {
  ...
  >
        ...
        {!raceId ? <p className="text-sm text-muted">No upcoming race to inspect.</p> : query.isPending ? <Skeleton className="h-32 w-full rounded-xl" /> : query.isError ? <p className="text-sm text-muted">Could not load inspections.</p> : rows.length === 0 ? <p className="text-sm text-muted">No runners declared for this race yet.</p> : <div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y divide-border">
                {rows.map((r) => <tr key={r.entryId}>
                    <td className="py-2.5 pr-3 font-mono text-muted">{r.laneNo ?? "\u2014"}</td>
                    <td className="py-2.5 pr-3">
                      <p className="font-medium text-ink">{r.horseName}</p>
                      <p className="text-xs text-muted">{r.jockeyName ?? "\u2014"}</p>
                    </td>
                    <td className="py-2.5 pr-3">
                      {r.weightVerified ? <span className="inline-flex items-center gap-1 text-success">
                          <Check size={14} /> Verified
                        </span> : <span className="inline-flex items-center gap-1 text-muted">
                          <X size={14} /> Pending
                        </span>}
                    </td>
                    <td className="py-2.5 pr-3">
...
export {
  RefereeDashboardPage as default
};
