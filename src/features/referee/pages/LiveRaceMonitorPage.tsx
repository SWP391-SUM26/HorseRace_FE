...
import type { InfractionType, SeverityLevel } from '../types';

/** ms → "MM:SS.d" elapsed clock. */
...
const QUICK_TAGS: InfractionType[] = ['BUMPING', 'INTERFERENCE', 'WHIP_USAGE'];

export default function LiveRaceMonitorPage() {
  const racesQuery = useRefereeRaces();
  const [raceId, setRaceId] = useState('');
  const [violationOpen, setViolationOpen] = useState(false);
  const [presetType, setPresetType] = useState<InfractionType>('WHIP_USAGE');
  ...
  function openViolation(type: InfractionType) {
    setPresetType(type);
  ...
  return (
    <>
      <PageHeader
        ...
        subtitle={race ? `${race.trackCondition ?? 'Live'} · officiating` : 'Monitor the running race in real time.'}
        actions={
          <div className="flex items-center gap-3">
            {live?.raceClockMs != null && (
      ...
      {liveQuery.isPending ? (
        ...
        <EmptyState title="No live feed" description="The live monitor is unavailable for this race." />
      ) : (
          ...
          <div className="space-y-6 lg:col-span-2">
            <Card>
              <CardBody className="flex flex-col gap-4">
                ...
                <div className="relative flex aspect-video items-center justify-center overflow-hidden rounded-xl border border-border bg-brand-900 text-white/60">
                  ...
                  {live.runningOrder[0]?.currentSpeedKph != null && (
                    <div className="absolute bottom-3 right-3 flex gap-4 rounded-lg bg-black/55 px-4 py-2 text-right text-white">
                      <div>
            ...
            <Card>
              <CardBody className="flex flex-col gap-3">
                ...
                {live.runningOrder.length === 0 ? (
                  <p className="text-sm text-muted">Waiting for the field…</p>
                ) : (
                  <ul className="flex flex-col gap-2">
                    {live.runningOrder.map((r, i) => (
                      <li
                        key={`${r.entryNo}-${i}`}
              ...
              <CardBody className="flex flex-col gap-3">
                ...
                {violationsQuery.isPending ? (
                  ...
                  <ul className="flex flex-col divide-y divide-border">
                    {log.map((v) => (
                      <li key={v.violationId} className="py-3">
...
function RecordViolationModal({
  ...
  onClose: () => void;
  raceId: string;
  presetType: InfractionType;
}) {
  const toast = useToast();
  const create = useCreateViolation(raceId);
  const [infractionType, setInfractionType] = useState<InfractionType>(presetType);
  const [severity, setSeverity] = useState<SeverityLevel>('MEDIUM');
  const [turnNo, setTurnNo] = useState('');
  const [remarks, setRemarks] = useState('');

  // Re-sync the type when opened from a quick tag.
  useEffect(() => {
