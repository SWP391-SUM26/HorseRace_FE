...
const STATUS_TONE: Record<RegistrationStatus, 'success' | 'danger' | 'warning' | 'neutral'> = {
  APPROVED: 'success',
  REJECTED: 'danger',
  SUBMITTED: 'warning',
...
const REVIEWABLE: RegistrationStatus[] = ['SUBMITTED', 'UNDER_REVIEW'];
...
export default function RegistrationManagementPage() {
  ...
  const listQuery = useRegistrations({ q: q || undefined, status: status || undefined, page });
          ...
          <Card>
            <CardBody className="p-0">
              {listQuery.isPending ? (
                ...
                <EmptyState title="Could not load registrations" description="Please reload the page." />
              ) : rows.length === 0 ? (
...
function Eligibility({ label, ok, note }: { label: string; ok: boolean; note?: string }) {
  return (
...
/** 409 = already reviewed, 400 = wrong state, 403 = not authorized. */
