...
import type { PenaltyType, SeverityLevel, ViolationListItem } from '../types';
...
function priorityLabel(sev: SeverityLevel): string | null {
  ...
  return (
    <>
      ...
      <div className="grid gap-6 lg:grid-cols-3">
          ...
          <Card>
            <CardBody className="p-0">
              {listQuery.isPending ? (
                ...
                <EmptyState title="Could not load violations" description="Please reload the page." />
              ) : rows.length === 0 ? (
                <EmptyState title="No inquiries" description="Nothing to review here." />
              ) : (
        ...
        <aside>
          {selected ? (
            <InquiryDetails key={selected.violationId} violationId={selected.violationId} />
          ) : (
            <Card>
              <CardBody>
...
function ViolationRow({
  ...
  return (
    <tr className={cn('transition-colors', active ? 'bg-brand-50' : 'hover:bg-subtle/60')}>
...
function InquiryDetails({ violationId }: { violationId: string }) {
          ...
          <>
            ...
            {(data.regulatoryRef || data.regulatoryText) && (
              <div className="rounded-xl border border-border bg-subtle/40 p-3">
                ...
                {data.regulatoryRef && (
                  <p className="mt-1 text-sm font-medium text-ink">{data.regulatoryRef}</p>
                )}
            ...
            {data.ruling ? (
              <div className="rounded-xl border border-brand-700 bg-brand-50 p-3">
                ...
                <p className="mt-1 text-sm font-medium text-ink">
                  ...
                  {data.ruling.penaltyType ? ` · ${humanize(data.ruling.penaltyType)}` : ''}
                  {data.ruling.penaltyValue ? ` (${data.ruling.penaltyValue})` : ''}
                </p>
                <p className="mt-0.5 text-xs text-muted">
                  {data.ruling.ruledByName ?? '—'}
                  {data.ruling.ruledAt ? ` · ${formatDate(data.ruling.ruledAt)}` : ''}
                </p>
              </div>
            ) : (
...
function RulingForm({ violationId }: { violationId: string }) {
  const toast = useToast();
