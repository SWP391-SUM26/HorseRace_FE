...
const INSPECTION_TONE: Record<InspectionStatus, 'success' | 'warning' | 'neutral'> = {
  CLEARED: 'success',
  VET_CHECK: 'warning',
  PENDING: 'neutral',
};
  ...
  return (
    <>
      ...
      {isPending ? (
        ...
        <EmptyState title="Could not load the dashboard" description="Please reload the page." />
      ) : (
        <>
...
function InspectionMiniTable({ raceId }: { raceId: string | null }) {
    ...
    <Card>
      <CardBody className="flex flex-col gap-3">
        ...
        {!raceId ? (
          ...
          <p className="text-sm text-muted">No runners declared for this race yet.</p>
        ) : (
