...
import { LiveRunningOrder, useLiveLeaderboard, useLiveRace } from "@/common/live";
...
function LiveRaceMonitorPage() {
  ...
  const racesQuery = useRefereeRaces({ refetchInterval: 1e4 });
  ...
  const race = useMemo(() => {
    ...
    const pick = assigned.find((r) => r.status === "RUNNING") ?? assigned[0] ?? null;
    return pick ? {
      raceId: pick.raceId,
  ...
  if (!race && !loading) {
    ...
    return <EmptyState
      ...
      description={noAssignments ? "You have no assigned races to officiate. When an admin assigns you to a race, it appears here to monitor live." : "This race is not available to monitor. Open a race from your assignments to monitor it live."}
    />;
  }
      ...
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          {running ? <span className="flex items-center gap-1.5 text-xs font-bold uppercase tracking-wide text-danger">
              <span className="h-1.5 w-1.5 animate-pulse rounded-full bg-danger" /> Live — Officiating
            </span> : <span className="text-xs font-bold uppercase tracking-wide text-muted">{race?.status ?? "Race"}</span>}
          <h1 className="mt-1 text-2xl font-semibold text-ink">{race?.name ?? race?.raceCode ?? "Live Race Monitor"}</h1>
  ...
  /> : <div className="flex h-full w-full items-center justify-center bg-gradient-to-br from-brand-800 to-slate-900">
            ...
            {live && (live.windSpeedKph != null || live.windDirection) && <div className="absolute right-3 top-3 flex items-center gap-1.5 rounded-lg bg-black/60 px-3 py-1.5 text-xs text-white">
                <Wind className="h-3.5 w-3.5" /> {live.windSpeedKph ?? "\u2014"} kph {live.windDirection ?? ""}
              </div>}
          </div>
    ...
    /* Live running order (from the officiating snapshot / leaderboard) */
          ...
          {liveQuery.isLoading ? <Skeleton className="h-40 w-full rounded-2xl" /> : <LiveRunningOrder
            ...
            <CardBody>
              ...
              <dl className="space-y-2 text-sm">
                <div className="flex items-center justify-between">
                  ...
                  <dd className="font-mono font-semibold tabular-nums text-ink">{fmtClock(live?.raceClockMs ?? null)}</dd>
                </div>
                <div className="flex items-center justify-between">
                  <dt className="text-muted">Wind speed</dt>
                  <dd className="text-ink">{live?.windSpeedKph != null ? `${live.windSpeedKph} kph` : "\u2014"}</dd>
                </div>
                <div className="flex items-center justify-between">
                ...
                <div className="flex items-center justify-between">
                  ...
                  <dd className="text-ink">{live?.runningOrder?.length ?? entries.length}</dd>
                </div>
              </dl>
            </CardBody>
          </Card>
