...
import type { InspectionListItem, InspectionStatus } from '../types';

const STATUS_TONE: Record<InspectionStatus, 'success' | 'warning' | 'danger'> = {
  CLEARED: 'success',
  PENDING: 'warning',
  VET_CHECK: 'danger',
};
...
export default function PreRaceInspectionPage() {
  ...
  function handleSubmitAll() {
    submitAll.mutate(undefined, {
      onSuccess: (res) =>
        res.blockedEntries.length > 0
          ? toast.info(`${res.submittedCount} cleared · ${res.blockedEntries.length} blocked`)
          : toast.success(`Submitted ${res.submittedCount} clearances`),
  ...
  return (
    <>
      <PageHeader
        ...
        subtitle={race ? `${race.name} · ${race.trackCondition ?? 'Inspection'}` : 'Official steward access'}
        actions={
      ...
      <div className="grid gap-6 lg:grid-cols-3">
          ...
          <Card>
            <CardBody className="p-0">
              ...
              {inspectionsQuery.isPending ? (
                ...
                <EmptyState title="Could not load inspections" description="Please reload the page." />
              ) : rows.length === 0 ? (
                <EmptyState title="No runners to inspect" />
              ) : (
        ...
        <aside>
          {selected && raceId ? (
            <ClearancePanel key={selected.entryId} raceId={raceId} row={selected} />
          ) : (
            <Card>
              <CardBody>
...
function ClearancePanel({ raceId, row }: { raceId: string; row: InspectionListItem }) {
  const toast = useToast();
  ...
  const [status, setStatus] = useState<InspectionStatus>(row.inspectionStatus);
    ...
    record.mutate(
      {
        ...
        stewardNote: note.trim() || undefined,
      },
      {
          ...
          {passport.isPending ? (
            ...
            <div className="mt-2 grid grid-cols-2 gap-3 text-sm">
              ...
              <Field
                label="Age / Sex"
                value={
                  passport.data?.ageYears != null
                    ? `${passport.data.ageYears}yo ${passport.data.genderWord}`
                    : (passport.data?.genderWord ?? '—')
                }
              />
