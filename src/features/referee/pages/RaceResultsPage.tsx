...
import type { OfficialityStatus, RaceResults, ResultInput } from '../types';
  ...
  return (
    <>
      <PageHeader
        ...
        subtitle={race ? `${race.name} · ${race.trackCondition ?? '—'}` : 'Record the order of finish, then publish for admin certification.'}
        actions={
      ...
      {reportableRaces.length === 0 ? (
        ...
        <EmptyState title="Select a race" description="Pick a finished race to record its results." />
      ) : (
        <ResultsBody
          raceId={raceId}
...
function ResultsBody({
  ...
  const seeded = useMemo(() => {
    ...
    for (const e of entriesQuery.data ?? []) {
      ...
      next[e.entryId] = {
        ...
        sec: o?.finishTimeMs != null ? String((o.finishTimeMs / 1000).toFixed(2)) : '',
      };
    }
    return next;
  ...
  function setCell(entryId: string, key: 'pos' | 'sec', value: string) {
  ...
  function buildPayload(): ResultInput[] {
    return (entriesQuery.data ?? [])
      .map((e) => {
        ...
        return {
          ...
          finishTimeMs: sec != null && !Number.isNaN(sec) ? Math.round(sec * 1000) : undefined,
        } as ResultInput;
      })
      .filter(Boolean) as ResultInput[];
  }
  ...
  return (
    <div className="grid gap-6 lg:grid-cols-3">
      <div className="space-y-6 lg:col-span-2">
          ...
          <Card>
            <CardBody>
              ...
              {entriesQuery.isPending ? (
                ...
                <p className="rounded-lg bg-subtle/60 px-3 py-3 text-sm text-muted">No runners entered for this race.</p>
              ) : (
                <ResultsGrid entries={entriesQuery.data} rows={rows} setCell={setCell} />
              )}
            </CardBody>
          </Card>
        )}
        ...
        {results && results.registeredNotEntered && results.registeredNotEntered.length > 0 && (
          <Card>
            <CardBody>
              ...
              <ul className="divide-y divide-border">
                {results.registeredNotEntered.map((r) => (
                  <li key={r.registrationId} className="flex items-center justify-between gap-3 py-2">
                    <div className="min-w-0">
                      <p className="truncate text-sm font-medium text-ink">{r.horseName ?? '—'}</p>
                      <p className="truncate text-xs text-muted">{r.ownerName ?? '—'}</p>
                    </div>
                    {r.registrationCode && (
                      <span className="shrink-0 text-xs tabular-nums text-muted">{r.registrationCode}</span>
                    )}
                  </li>
                ))}
              </ul>
...
function LockBanner({ isOfficial }: { isOfficial: boolean }) {
  return (
...
function ResultsGrid({
  ...
  entries: { entryId: string; entryNo: number | null; horseName: string }[];
  rows: Record<string, { pos: string; sec: string }>;
  setCell: (entryId: string, key: 'pos' | 'sec', value: string) => void;
}) {
  return (
 ...
 */
function PublishPanel({ raceId, buildPayload }: { raceId: string; buildPayload: () => ResultInput[] }) {
...
function AdminSavePanel({ raceId, buildPayload }: { raceId: string; buildPayload: () => ResultInput[] }) {
  const toast = useToast();
...
function FinishTable({
  ...
  function raise(resultId: string) {
...
function AdminCertificationNote({ isOfficial }: { isOfficial: boolean }) {
  return (
    <Card>
      <CardBody className="flex flex-col gap-2">
        ...
        {isOfficial ? (
          <p className="text-sm text-success">These results are certified OFFICIAL.</p>
        ) : (
