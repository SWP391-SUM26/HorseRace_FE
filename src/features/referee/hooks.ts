...
import type { CertifyResultsRequest, CreateViolationRequest, InspectionRequest, ResultInput, RulingRequest } from './types';
 ...
 */
export function useRefereeRaces(options: { refetchInterval?: number } = {}) {
  return useQuery({
...
export function useRefereeRaceById(raceId: string | null) {
  return useQuery({
...
// ---------- Document review (CN2) ----------
export function useEntryReviews(raceId: string | null) {
  return useQuery({
...
export function useAcceptEntry(raceId: string) {
  const qc = useQueryClient();
  return useMutation({
    mutationFn: (entryId: string) => acceptEntry(raceId, entryId),
    onSuccess: () => qc.invalidateQueries({ queryKey: ['referee', 'entry-reviews', raceId] }),
  });
}

export function useRejectEntry(raceId: string) {
  const qc = useQueryClient();
  return useMutation({
    mutationFn: ({ entryId, reason }: { entryId: string; reason: string }) => rejectEntry(raceId, entryId, reason),
    onSuccess: () => qc.invalidateQueries({ queryKey: ['referee', 'entry-reviews', raceId] }),
  });
}
...
export function useUpdateViolation() {
  const qc = useQueryClient();
  return useMutation({
    mutationFn: (v: { violationId: string; body: CreateViolationRequest }) => updateViolation(v.violationId, v.body),
...
export function useDeleteViolation() {
  const qc = useQueryClient();
  return useMutation({
    mutationFn: (violationId: string) => deleteViolation(violationId),
    onSuccess: () => qc.invalidateQueries({ queryKey: ['referee', 'violations'] }),
  });
}
...
export function useRecordResults(raceId: string) {
  const qc = useQueryClient();
  return useMutation({
    mutationFn: (vars: { results: ResultInput[] }) => recordResults(raceId, vars.results),
    onSuccess: () => qc.invalidateQueries({ queryKey: ['referee', 'results', raceId] }),
  });
}

/** CN3: email the referee a fresh OTP for this race. */
export function useRequestRefereeCode(raceId: string) {
...
export function useSubmitReport(raceId: string) {
  const qc = useQueryClient();
  return useMutation({
    mutationFn: (vars: { otp: string; results: ResultInput[]; violations: CreateViolationRequest[] }) =>
      submitReport(raceId, vars),
    onSuccess: () => {
...
export function useFlagInquiry(raceId: string) {
  const qc = useQueryClient();
  return useMutation({
    mutationFn: (resultId: string) => flagInquiry(raceId, resultId),
    onSuccess: () => qc.invalidateQueries({ queryKey: ['referee', 'results', raceId] }),
  });
}
...
export function useDeleteResult(raceId: string) {
  const qc = useQueryClient();
  return useMutation({
    mutationFn: (resultId: string) => deleteResult(raceId, resultId),
    onSuccess: () => qc.invalidateQueries({ queryKey: ['referee', 'results', raceId] }),
  });
}
...
export function useAcceptTournamentInvitation() {
  const qc = useQueryClient();
  return useMutation({
    mutationFn: (id: string) => acceptTournamentInvitation(id),
...
export function useRejectTournamentInvitation() {
  const qc = useQueryClient();
  return useMutation({
    mutationFn: (id: string) => rejectTournamentInvitation(id),
    onSuccess: () => qc.invalidateQueries({ queryKey: ['referee', 'tournament-invitations'] }),
  });
}
...
export function useAcceptRaceAssignment() {
  const qc = useQueryClient();
  return useMutation({
    mutationFn: (id: string) => acceptRaceAssignment(id),
...
export function useDeclineRaceAssignment() {
  const qc = useQueryClient();
  return useMutation({
    mutationFn: (v: { id: string; reason?: string }) => declineRaceAssignment(v.id, v.reason),
    onSuccess: () => qc.invalidateQueries({ queryKey: ['referee', 'race-assignments'] }),
  });
}
...
export function useRequestApplicationInfo() {
  const qc = useQueryClient();
  return useMutation({
    mutationFn: ({ id, note }: { id: string; note: string }) => requestApplicationInfo(id, note),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ['referee', 'applications'] });
