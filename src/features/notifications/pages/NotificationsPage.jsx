...
function NotificationsPage() {
  ...
  />
      ...
      {list.isPending ? <div className="flex flex-col gap-3">
          {Array.from({ length: 4 }).map((_, i) => <Skeleton key={i} className="h-20 w-full rounded-2xl" />)}
        </div> : list.isError ? <Card className="p-0">
          <EmptyState
    title="Couldn't load notifications"
    description="Something went wrong reaching the server — please retry."
    action={<Button variant="secondary" size="sm" onClick={() => list.refetch()}>Retry</Button>}
  />
        </Card> : items.length === 0 ? <Card className="p-0">
          <EmptyState title="No notifications yet" description="You're all caught up — new alerts will show up here." />
        </Card> : <ul className="flex flex-col gap-3">
          {items.map((n) => <li key={n.notificationId}>
              <button
    type="button"
    onClick={() => {
    ...
    className={cn(
      ...
      n.isRead ? "bg-surface hover:bg-subtle" : "bg-brand-50/40 hover:bg-brand-50"
    )}
  >
                <span className="flex items-center gap-2">
                  {!n.isRead && <span aria-label="Unread" className="h-2 w-2 shrink-0 rounded-full bg-brand-700" />}
                  <span className={cn("text-sm", n.isRead ? "font-medium text-ink" : "font-semibold text-ink")}>
                    {n.title}
                  </span>
                  {n.createdAt && <span className="ml-auto shrink-0 text-xs text-muted">{formatDate(n.createdAt)}</span>}
                </span>
