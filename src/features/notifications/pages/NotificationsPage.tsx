...
/** Full-page Notifications Center — shared by every authenticated role (reached from the bell). */
            ...
            <li key={n.notificationId}>
              ...
              >
                <span className="flex items-center gap-2">
                  {!n.isRead && (
                    <span aria-label="Unread" className="h-2 w-2 shrink-0 rounded-full bg-brand-700" />
                  )}
                  <span className={cn('text-sm', n.isRead ? 'font-medium text-ink' : 'font-semibold text-ink')}>
                    {n.title}
                  </span>
                  {n.createdAt && (
                    <span className="ml-auto shrink-0 text-xs text-muted">{formatDate(n.createdAt)}</span>
                  )}
                </span>
