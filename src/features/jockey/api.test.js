...
import { buildTrophies, buildWinTrend, humanize, toUpdateJockeyProfileRequest } from "./api";
...
describe("buildWinTrend", () => {
  const now = /* @__PURE__ */ new Date("2026-06-15T00:00:00Z");
  it("returns 12 trailing months ending with the current month", () => {
    const points = buildWinTrend([], now);
    expect(points).toHaveLength(12);
    expect(points[11].month).toBe("Jun");
    expect(points[0].month).toBe("Jul");
  });
  it("counts only wins (finishPosition === 1) into the matching month bucket", () => {
    const points = buildWinTrend(
      [
        ...
        ride({ date: "2026-05-10T00:00:00Z", finishPosition: 2 })
        // not a win
      ],
      now
    );
    expect(points[11].wins).toBe(2);
    expect(points[10].wins).toBe(0);
  });
  it("ignores wins outside the trailing 12-month window", () => {
    const points = buildWinTrend([ride({ date: "2024-10-24T00:00:00Z", finishPosition: 1 })], now);
...
describe("toUpdateJockeyProfileRequest", () => {
  it("keeps filled fields and drops empty/undefined (partial update)", () => {
    ...
    expect(body).toEqual({ bodyWeight: 55, ridingStyle: "Stalker", baseFee: 0, prizePercent: 10 });
  ...
  it("trims string fields it keeps", () => {
    const body = toUpdateJockeyProfileRequest({ ridingStyle: "  Closer  ", licenseNo: " LIC-9 " });
...
describe("buildTrophies", () => {
  it("keeps only races won, newest first, deduped by name+year", () => {
    const trophies = buildTrophies([
      ride({ raceName: "Derby", date: "2024-05-01T00:00:00Z", finishPosition: 1 }),
      ride({ raceName: "Oaks", date: "2025-06-01T00:00:00Z", finishPosition: 1 }),
      ride({ raceName: "Derby", date: "2024-05-01T00:00:00Z", finishPosition: 1 }),
      // dup
      ride({ raceName: "Sprint", date: "2025-01-01T00:00:00Z", finishPosition: 3 })
      // not a win
    ]);
