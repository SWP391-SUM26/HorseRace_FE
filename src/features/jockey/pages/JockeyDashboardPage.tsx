 ...
 */
function raceGradeBadge(inv: Invitation): { tone: 'info' | 'neutral'; label: string } {
  const grades: { tone: 'info' | 'neutral'; label: string }[] = [
...
function InvitationRow({ invitation, onAccept, onDecline, accepting, declining }: RowProps) {
...
/** Friendly toast text; 409 means the invitation was already actioned. */
function errorMessage(err: unknown): string {
  if (isAxiosError(err) && err.response?.status === 409) {
    return 'Lời mời này đã được xử lý.';
  }
