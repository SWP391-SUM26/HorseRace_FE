...
const EMPTY_LABEL = {
  INVITED: "Kh\xF4ng c\xF3 l\u1EDDi m\u1EDDi \u0111ang ch\u1EDD",
  ACCEPTED: "Kh\xF4ng c\xF3 l\u1EDDi m\u1EDDi \u0111\xE3 nh\u1EADn",
  DECLINED: "Kh\xF4ng c\xF3 l\u1EDDi m\u1EDDi \u0111\xE3 t\u1EEB ch\u1ED1i",
  CANCELLED: "Kh\xF4ng c\xF3 l\u1EDDi m\u1EDDi"
};
function formatUsd(amount) {
  return `$${Math.round(amount).toLocaleString("en-US")}`;
}
function JockeyInvitationsPage() {
  const { user } = useAuth();
  ...
  function handleWithdraw() {
    ...
    withdraw.mutate(withdrawTarget.id, {
      onSuccess: () => {
        toast.success("\u0110\xE3 r\xFAt kh\u1ECFi ch\u1EB7ng \u0111ua");
  ...
  />
      ...
      <div className="grid gap-6 lg:grid-cols-3">
        <div className="flex flex-col gap-4 lg:col-span-2">
          ...
          {isPending ? <InvitationsSkeleton /> : isError ? <EmptyState
    title="Không tải được lời mời"
    description="Vui lòng tải lại trang để thử lại."
  /> : !data || data.length === 0 ? <EmptyState title={EMPTY_LABEL[tab]} /> : <div className="flex flex-col gap-4">
              {data.map((inv) => <InvitationCard
    key={inv.id}
    invitation={inv}
...
function InvitationDetailsModal({
  ...
  return <Modal
    ...
    footer={<Button variant="secondary" onClick={onClose}>
          Đóng
        </Button>}
  >
      {inv && badge && <div className="flex flex-col gap-5">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <DetailItem
    label="Entry"
    value={inv.entryNo != null ? `#${inv.entryNo} \xB7 ${inv.entryCode}` : inv.entryCode}
  />
          </div>
...
function WithdrawConfirmModal({
  ...
  >
      {invitation && <p className="text-sm text-ink">
...
function InvitationInsights({ enabled }) {
  ...
  return <Card>
      <CardBody className="flex flex-col gap-6">
        ...
        {isPending ? <div className="flex flex-col gap-4">
          ...
          </div> : isError || !data ? <p className="text-sm text-muted">Không tải được số liệu.</p> : <>
            <div>
              <p className="text-xs font-medium uppercase tracking-wide text-muted">
  ...
  />
            ...
            <div>
              ...
              {data.mostActiveOwners.length === 0 ? <p className="mt-2 text-sm text-muted">Chưa có dữ liệu.</p> : <ul className="mt-2 flex flex-col gap-2">
                  {data.mostActiveOwners.map((owner) => <li
...
function errorMessage(err) {
  if (isAxiosError(err)) {
    ...
    if (status === 409) return "L\u1EDDi m\u1EDDi n\xE0y \u0111\xE3 \u0111\u01B0\u1EE3c x\u1EED l\xFD.";
    if (status === 400) return "Ch\u1EC9 c\xF3 th\u1EC3 r\xFAt nh\u1EEFng ch\u1EB7ng \u0111\xE3 nh\u1EADn.";
    if (status === 403) return "B\u1EA1n kh\xF4ng th\u1EC3 thao t\xE1c v\u1EDBi l\u1EDDi m\u1EDDi n\xE0y.";
  }
  return "C\xF3 l\u1ED7i x\u1EA3y ra. Vui l\xF2ng th\u1EED l\u1EA1i.";
}
export {
  JockeyInvitationsPage as default
};
