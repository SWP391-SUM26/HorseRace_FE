...
function WinTrendCard({
  ...
  return <Card>
      ...
      <CardBody>
        {loading ? <Skeleton className="h-48 w-full rounded-xl" /> : error ? <p className="flex h-48 items-center justify-center text-sm text-muted">
            Không tải được dữ liệu.
          </p> : totalWins === 0 ? <div className="flex h-48 items-center justify-center">
            <EmptyState
    title="Chưa có chiến thắng trong 12 tháng qua"
    description="Biểu đồ sẽ hiển thị số trận thắng theo tháng khi có kết quả mới."
  />
          </div> : <div className="flex h-48 items-end justify-between gap-2">
            {points.map((m, i) => {
    const highlight = m.wins > 0 && m.wins >= maxWins;
    return <div key={`${m.month}-${i}`} className="flex h-full flex-1 flex-col items-center gap-2">
...
function TrophyCabinetCard({
  ...
  return <Card>
      ...
      <CardBody>
        {loading ? <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
            {Array.from({ length: 4 }).map((_, i) => <Skeleton key={i} className="h-28 w-full rounded-2xl" />)}
          </div> : error ? <p className="text-sm text-muted">Không tải được dữ liệu.</p> : trophies.length === 0 ? <EmptyState
    title="Chưa có cúp"
  ...
  >
      ...
      <CardBody className="flex flex-1 flex-col gap-1">
        {loading ? <div className="flex flex-col gap-2">
          ...
          </div> : error ? <p className="text-sm text-muted">Không tải được bảng xếp hạng.</p> : entries.length === 0 ? <p className="text-sm text-muted">Chưa có dữ liệu xếp hạng.</p> : <ul className="flex flex-col gap-1">
            {entries.map((row) => {
