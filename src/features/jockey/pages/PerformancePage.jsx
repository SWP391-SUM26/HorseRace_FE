        ...
        {statsQuery.isPending ? (
          ...
          <>
            <StatCard
              ...
              value={stats.careerWins}
              hint="All-time"
            />
            <StatCard
              label="Top 3 Finish Rate"
              value={`${stats.top3Rate}%`}
              hint="This season"
            />
            <StatCard
              label="Avg Placement"
...
function WinTrendCard({ points, loading, error }) {
...
function TrophyCabinetCard({ trophies, loading, error }) {
  return (
    <Card>
      <CardHeader className="flex items-center justify-between">
...
function RankingsCard({ entries, loading, error, currentUserId }) {
      ...
      <CardBody className="flex flex-1 flex-col gap-1">
        {loading ? (
          ...
          <ul className="flex flex-col gap-1">
            {entries.map((row) => {
              const isCurrent =
                !!currentUserId && row.jockeyUserId === currentUserId;
              return (
                <li
                  ...
                  className={cn(
                    ...
                    isCurrent
                      ? "bg-brand-50 ring-1 ring-brand-700/30"
                      : "hover:bg-subtle/60",
                  )}
                >
                  <span
