...
import { Avatar, Badge, Button, Card, CardBody, EmptyState, Skeleton, StatCard } from "@/common/ui";
...
function JockeyDashboardPage() {
  ...
  function handleAccept(id) {
    accept.mutate(id, {
      onSuccess: () => toast.success("\u0110\xE3 nh\u1EADn l\u1EDDi m\u1EDDi"),
      onError: (err) => toast.error(errorMessage(err))
    });
  }
  function handleDecline(id) {
    reject.mutate(id, {
      onSuccess: () => toast.success("\u0110\xE3 t\u1EEB ch\u1ED1i"),
      onError: (err) => toast.error(errorMessage(err))
    });
  }
    ...
    /* KPI row — all REAL from GET /jockeys/me/stats */
      ...
      <div className="grid gap-4 sm:grid-cols-3">
        {statsQuery.isPending ? <>
            ...
            <Skeleton className="h-28 w-full rounded-2xl" />
            <Skeleton className="h-28 w-full rounded-2xl" />
          </> : statsQuery.isError || !stats ? <Card className="flex h-28 items-center justify-center p-6 text-sm text-muted sm:col-span-3">
  ...
  />
    ...
    hint={`${stats.wins} wins \xB7 ${stats.places} places`}
  />
            {
    /* dark emerald card — NOT <Card> (bg override loses class-order) */
            ...
            <div className="rounded-2xl border border-brand-700 bg-brand-800 p-6 text-white shadow-sm">
              ...
              <p className="mt-1 text-3xl font-semibold">{formatUsd(stats.seasonEarnings)}</p>
              <p className="mt-2 text-xs text-white/70">Career: {formatUsd(stats.careerEarnings)}</p>
            </div>
          </>}
      </div>
  ...
  >
          ...
          {invitedQuery.isPending ? <InvitationsSkeleton /> : invitedQuery.isError ? <EmptyState
    title="Không tải được lời mời"
    description="Vui lòng tải lại trang để thử lại."
  /> : !invitedQuery.data || invitedQuery.data.length === 0 ? <EmptyState title="Không có lời mời đang chờ" /> : <div className="flex flex-col gap-4">
              {invitedQuery.data.map((inv) => <InvitationRow
    key={inv.id}
    invitation={inv}
        ...
        <aside>
          <Card>
            <CardBody className="flex flex-col gap-4">
              ...
              {upcomingQuery.isPending ? <div className="flex flex-col gap-3">
                  {Array.from({ length: 3 }).map((_, i) => <Skeleton key={i} className="h-14 w-full rounded-xl" />)}
                </div> : upcomingQuery.isError ? <p className="text-sm text-muted">Không tải được lịch đua.</p> : !upcomingQuery.data || upcomingQuery.data.length === 0 ? <p className="text-sm text-muted">Chưa có lịch đua sắp tới.</p> : <ul className="flex flex-col gap-3">
                  {upcomingQuery.data.map((ride) => <li
    key={ride.id}
    className="rounded-xl border border-border bg-subtle/40 p-3"
  >
                      <p className="text-xs font-medium text-muted">
                        {ride.date ? formatDate(ride.date) : "Ch\u01B0a l\xEAn l\u1ECBch"}
                      </p>
                      <p className="mt-0.5 text-sm font-medium text-ink">{ride.raceName}</p>
...
function InvitationRow({ invitation, onAccept, onDecline, accepting, declining }) {
...
function errorMessage(err) {
  if (isAxiosError(err) && err.response?.status === 409) {
    return "L\u1EDDi m\u1EDDi n\xE0y \u0111\xE3 \u0111\u01B0\u1EE3c x\u1EED l\xFD.";
  }
  return "C\xF3 l\u1ED7i x\u1EA3y ra. Vui l\xF2ng th\u1EED l\u1EA1i.";
}
export {
  JockeyDashboardPage as default
};
