  ...
  const onSubmit = (data) => {
    ...
    update.mutate(body, {
      onSuccess: () => {
        if (cleared.length) {
          toast.info(
            `Đã lưu. Không thể xoá trắng: ${cleared.join(", ")} — giữ giá trị cũ.`,
          );
        } else {
            ...
            <Card>
              <CardBody className="flex flex-col gap-5">
                ...
                <dl className="flex flex-col gap-3 text-sm">
                  ...
                  <ReadOnly
                    label="Career Wins"
                    value={detail.winCount ?? "—"}
                  />
                  <ReadOnly
                    label="Kinh nghiệm"
                    value={
                      detail.experienceYrs != null
                        ? `${detail.experienceYrs} năm`
                        : "—"
                    }
                  />
                  ...
                  <ReadOnly
                    label="Đánh giá"
                    value={detail.rating != null ? `${detail.rating} / 5` : "—"}
                  />
                  <ReadOnly
                    label="Cúp gần nhất"
                    value={detail.lastTrophy ?? "—"}
                  />
                </dl>
