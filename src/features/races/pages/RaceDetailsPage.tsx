...
const STATUS_TONE: Record<string, Tone> = {
  SCHEDULED: 'info', OPEN: 'warning', CLOSED: 'neutral', RUNNING: 'success',
  FINISHED: 'neutral', OFFICIAL: 'success', CANCELLED: 'danger',
};
const STATUS_LABEL: Record<string, string> = {
  SCHEDULED: 'Scheduled', OPEN: 'Open', CLOSED: 'Closed', RUNNING: 'Running',
  FINISHED: 'Finished', OFFICIAL: 'Official', CANCELLED: 'Cancelled',
};

/** Owner Race Calendar — the real list of races across tournaments. */
      ...
      <Card>
        <CardBody className="p-0">
          {isPending ? (
            ...
            <EmptyState title="Không tải được lịch đua" description="Vui lòng thử lại sau." />
          ) : rows.length === 0 ? (
            <EmptyState title="Chưa có cuộc đua nào" description="Lịch đua sẽ hiển thị ở đây khi có race." />
          ) : (
            <div className="overflow-x-auto">
              <table className="w-full min-w-[820px] text-sm">
                ...
                <tbody className="divide-y divide-border">
                  {rows.map((r) => (
                    <tr key={r.raceId} className="hover:bg-subtle/40">
