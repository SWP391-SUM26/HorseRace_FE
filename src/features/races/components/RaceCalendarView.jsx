 ...
 * Race Calendar.
 *
 * - `canManage` (admin) unlocks "Add New Event" and click-to-manage.
 * - `restrictToRaceIds` is owner mode: `undefined` shows all races (admin),
 *   `null` means restricted but the owner's data is still loading.
 * - `confirmHref` (owner mode) links each upcoming race card to its
 *   Confirm-Participation readiness view.
 */
export function RaceCalendarView({
  ...
  useEffect(() => {
    ...
    const dated = scopedRows
      .map((r) => parseDate(r.scheduledStartAt))
      .filter((d) => !!d)
      .sort((a, b) => +a - +b);
  ...
  const grades = useMemo(
    () =>
      [
        ...new Set(
          scopedRows.map((r) => gradeFor(r.raceType)?.short).filter((g) => !!g),
        ),
      ].sort(),
    [scopedRows],
  );
  ...
  const selectedPurse = selectedRaces.reduce(
    (sum, r) => sum + (r.totalPurse ?? 0),
    0,
  );
    ...
    <div className="flex flex-col gap-6">
      <div>
        ...
        <p className="mt-1 text-sm text-muted">
          {subtitle ?? "Manage and track global racing events."}
        </p>
      </div>
      ...
      {noScoped ? (
        <Card>
          <CardBody>
            <EmptyState
              title={emptyLabel ?? "No races yet"}
              description="Register a horse for a tournament race and it will appear on your calendar."
            />
          </CardBody>
        </Card>
      ) : (
      <div className="grid gap-6 lg:grid-cols-3">
        ...
        <div className="lg:col-span-2">
          ...
          {isError ? (
            <Card className="mt-4">
              <CardBody>
                <EmptyState title="Couldn't load races" description="Please reload the page." />
              </CardBody>
            </Card>
          ) : (
...
function DetailPanel({
  ...
  return (
    <Card className="lg:sticky lg:top-4 flex max-h-[calc(100dvh-7rem)] flex-col">
      {!selected ? (
        <CardBody>
          <EmptyState title="Select a date" description="Pick a day on the calendar to see its events." />
        </CardBody>
      ) : (
        <>
