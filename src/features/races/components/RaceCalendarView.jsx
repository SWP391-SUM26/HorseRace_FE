...
const isFinished = (s) => s === "FINISHED" || s === "OFFICIAL" || s === "CANCELLED";
function gradeFor(raceType) {
...
function RaceCalendarView({
  ...
  const isRestricted = restrictToRaceIds !== void 0;
  ...
  useEffect(() => {
    ...
    const dated = scopedRows.map((r) => parseDate(r.scheduledStartAt)).filter((d) => !!d).sort((a, b) => +a - +b);
  ...
  const venues = useMemo(
    () => [...new Set(scopedRows.map((r) => r.venue).filter((v) => !!v))].sort(),
    [scopedRows]
  );
  const grades = useMemo(
    () => [...new Set(scopedRows.map((r) => gradeFor(r.raceType)?.short).filter((g) => !!g))].sort(),
    [scopedRows]
  );
  const cells = useMemo(() => {
  ...
  return <div className="flex flex-col gap-6">
      ...
      {noScoped ? <Card><CardBody><EmptyState title={emptyLabel ?? "No races yet"} description="Register a horse for a tournament race and it will appear on your calendar." /></CardBody></Card> : <div className="grid gap-6 lg:grid-cols-3">
        {
    /* ── Calendar (left) ── */
  }
  ...
  />
    ...
    /* Grid */
          ...
          {isError ? <Card className="mt-4"><CardBody><EmptyState title="Couldn't load races" description="Please reload the page." /></CardBody></Card> : <Card className="mt-4 overflow-hidden">
              <div className="grid grid-cols-7 border-b border-border bg-subtle/40">
                {WEEKDAYS.map((d) => <div key={d} className="px-2 py-2 text-center text-xs font-semibold uppercase tracking-wide text-muted">{d}</div>)}
              </div>
    ...
    >
      ...
      return <span
        ...
        className={cn(
          ...
          s.tone === "danger" ? "bg-red-50 text-danger" : s.tone === "success" ? "bg-brand-50 text-success" : s.tone === "neutral" ? "bg-subtle text-muted" : "bg-amber-50 text-warning"
        )}
        title={r.name}
      >
                        ...
                        {races.length > 2 && <span className="px-1 text-[10px] font-medium text-muted">+{races.length - 2} more races</span>}
                      </div>
                    </button>;
  })}
              </div>
...
function DetailPanel({
  ...
  return <Card className="lg:sticky lg:top-4 flex max-h-[calc(100dvh-7rem)] flex-col">
      {!selected ? <CardBody><EmptyState title="Select a date" description="Pick a day on the calendar to see its events." /></CardBody> : <>
          <div className="flex items-start justify-between border-b border-border p-5 pb-4">
            <div>
              ...
              <div className="mt-3 flex flex-wrap gap-2">
                ...
                {totalPurse > 0 && <span className="inline-flex items-center gap-1 rounded-md bg-amber-50 px-2 py-1 text-xs font-semibold text-warning">
          ...
          <div className="flex-1 overflow-y-auto p-5">
            {isPending ? <div className="flex flex-col gap-4">{Array.from({ length: 2 }).map((_, i) => <Skeleton key={i} className="h-48 w-full rounded-2xl" />)}</div> : races.length === 0 ? <EmptyState title="No events" description="No races scheduled on this day." /> : <ul className="flex flex-col gap-4">
                {races.map((r, i) => <EventCard
    key={r.raceId}
    race={r}
    index={i}
    clickable={canManage}
  ...
  />)}
          ...
          {canManage && <div className="border-t border-border p-4">
              <Button className="w-full" onClick={onAddEvent}><Plus className="h-4 w-4" /> Add New Event</Button>
            </div>}
        </>}
    </Card>;
}
function EventCard({ race, index, clickable, onClick, confirmHref }) {
  ...
  if (finished) {
    ...
    >
          ...
          {race.venue && <p className="mt-1 flex items-center gap-1 text-xs text-muted"><MapPin className="h-3 w-3" />{race.venue}</p>}
        </Wrapper>
      </li>;
  }
  return <li>
  ...
  >
        ...
        <div className="p-4">
          ...
          {entries > 0 && <div className="mt-3">
              <p className="mb-1.5 text-xs font-medium text-muted">Featured Entries</p>
              <div className="flex items-center gap-1.5">
                {Array.from({ length: Math.min(3, entries) }, (_, k) => <span key={k} className={cn(
    ...
    k === 0 ? "bg-brand-700 text-white" : "bg-subtle text-ink"
          ...
          {start && <p className="mt-3 text-xs text-muted">Post time {fmtTime(start)}</p>}
