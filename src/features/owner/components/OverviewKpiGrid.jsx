...
function OverviewKpiGrid({ kpis }) {
  ...
  />
    ...
    value={<span className="inline-flex items-center gap-2">
            {usd(kpis.netProfit)}
            {kpis.netProfitTrend != null && <span className="inline-flex items-center gap-0.5 text-sm font-medium text-success">
                <TrendingUp size={14} />+{kpis.netProfitTrend}%
              </span>}
          </span>}
    hint={kpis.margin != null ? `Margin ${kpis.margin}%` : void 0}
    icon={<Wallet size={18} />}
  />
    ...
    hint={kpis.pendingEtaDays != null ? `${kpis.pendingCount} pending \xB7 ~${kpis.pendingEtaDays} days` : `${kpis.pendingCount} pending`}
    icon={<Clock size={18} />}
  />
    </div>;
}
