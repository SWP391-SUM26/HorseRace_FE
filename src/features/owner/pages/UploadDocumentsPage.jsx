      ...
      <CardBody className="flex flex-col gap-4">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div>
            ...
            <p className="text-xs text-muted">ID, licences and other papers that identify you as the owner.</p>
          </div>
          <div>
            <input
    ref={inputRef}
...
function HorseDocsSection() {
      ...
      <CardBody className="flex flex-col gap-4">
        <div className="flex flex-wrap items-end justify-between gap-3">
          <div>
            ...
            <p className="text-xs text-muted">Passports, health certificates and other papers per horse.</p>
          </div>
          <div className="flex items-end gap-2">
            {horses.length > 0 && <Select
    label="Horse"
    value={selected}
  ...
  />}
    ...
    className="hidden"
  />
            <Button size="sm" loading={upload.isPending} disabled={!selected} onClick={() => inputRef.current?.click()}>
              <Upload size={15} /> Upload document
            </Button>
          </div>
        </div>

        {horsesQ.isPending ? <Skeleton className="h-16 w-full rounded-xl" /> : horses.length === 0 ? <EmptyState title="No horses yet" description="Register a horse in your stable to upload its documents." /> : <DocList query={docsQ} emptyText="No documents uploaded for this horse yet." />}
      </CardBody>
    </Card>;
}
function DocList({
  ...
  if (query.isError) return <p className="text-sm text-danger">Couldn't load documents.</p>;
  ...
  return <>
    ...
    return <li key={d.attachmentId}>
              <button
      type="button"
      onClick={() => viewer.view(`/owner/documents/${d.attachmentId}/download`, d.fileName)}
...
export {
  UploadDocumentsPage as default
};
