...
function regTime(r: OwnerRegistration): number {
  const t = r.updatedAt ?? r.createdAt;
  const ms = t ? Date.parse(t) : NaN;
  return Number.isNaN(ms) ? 0 : ms;
}

function ReadinessRow({ label, ready, reason }: { label: string; ready: boolean; reason?: string }) {
  return (
...
export default function ConfirmParticipationPage() {
  ...
  // Prefer the most recent registration so a stale REJECTED row can't shadow a re-approved one.
  ...
  const backLink = (
    <Link to="/app/owner/races" className="inline-flex items-center gap-1 text-sm text-brand-700 hover:underline">
  ...
  // A failed fetch must not read as "not entered" — surface a distinct, retryable error state.
  if (entries.isError) {
    return (
      <div className="mx-auto w-full max-w-2xl">
        {backLink}
