...
function OwnerDocsSection() {
  ...
  function onPick(file: File | undefined) {
    if (!file) return;
    upload.mutate(file, {
      onSuccess: () => { toast.success('Document uploaded'); if (inputRef.current) inputRef.current.value = ''; },
    ...
    <Card>
      <CardBody className="flex flex-col gap-4">
        ...
        {horsesQ.isPending ? (
          ...
          <EmptyState title="No horses yet" description="Register a horse in your stable to upload its documents." />
        ) : (
          <DocList query={docsQ} emptyText="No documents uploaded for this horse yet." />
        )}
      </CardBody>
    </Card>
  );
...
function DocList({
  ...
  query: { isPending: boolean; isError: boolean; data?: AttachmentResponse[] };
  emptyText: string;
}) {
