...
import { Avatar, Badge, Button, Card, CardBody, EmptyState, Input, Skeleton, Tabs } from "@/common/ui";
...
import { useTournament, useTournamentEntries, useTournamentRaces, useTournaments } from "../hooks";
...
function TournamentCatalogPage() {
  const PAGE_SIZE = 4;
  const listQuery = useTournaments();
  const tournaments = useMemo(
    () => (listQuery.data ?? []).filter((t) => t.status === "PUBLISHED" || t.status === "REGISTRATION_OPEN"),
    [listQuery.data]
  );
  ...
  const selected = useMemo(() => {
    ...
    return tournaments.find((t) => t.tournamentId === selectedId) ?? tournaments.find((t) => t.status === "REGISTRATION_OPEN") ?? tournaments[0];
  }, [tournaments, selectedId]);
  const filtered = useMemo(
    () => tournaments.filter((t) => t.name.toLowerCase().includes(q.trim().toLowerCase())),
    [tournaments, q]
  );
  ...
  return <div className="flex flex-col gap-6">
      ...
      {tab === "registrations" ? <OwnerRegistrationsPanel /> : <div className="grid gap-6 lg:grid-cols-5">
      {
    /* ── Catalog (left) ── */
  }
      <div className="lg:col-span-2">
        <h1 className="text-2xl font-semibold text-ink">Tournament Catalog</h1>
        <p className="mt-1 text-sm text-muted">Discover elite racing events worldwide.</p>
    ...
    setPage(0);
        ...
        {listQuery.isPending ? <div className="mt-4 grid grid-cols-1 gap-4 sm:grid-cols-2">
            {Array.from({ length: 4 }).map((_, i) => <Skeleton key={i} className="h-48 w-full rounded-2xl" />)}
          </div> : listQuery.isError ? <Card className="mt-4"><CardBody><EmptyState title="Couldn't load tournaments" description="Please reload the page." /></CardBody></Card> : filtered.length === 0 ? <Card className="mt-4"><CardBody><EmptyState title="No tournaments" description={q ? "No events match your search." : "No racing events to show yet."} /></CardBody></Card> : <>
    ...
    /* ── Detail (right) ── */
      ...
      <div className="lg:col-span-3">
        {selected ? <DetailPanel tournament={selected} /> : <Card><CardBody><EmptyState title="Select a tournament" description="Pick an event to see its details." /></CardBody></Card>}
      </div>
      </div>}
    </div>;
}
function CatalogCard({ t, index, active, onSelect }) {
  ...
  >
      ...
      <div className="p-3">
        ...
        <p className="mt-1 flex items-center gap-1 text-xs text-muted"><CalendarDays size={12} />{t.startDate ? formatDate(t.startDate) : "\u2014"}</p>
  ...
  return <Card className="overflow-hidden">
    ...
    /* Hero */
      ...
      <div className="relative h-48 w-full">
        <img src={t.imageUrl || heroHorses} alt="" className="h-48 w-full object-cover" />
        <div className="absolute inset-0 bg-gradient-to-t from-black/85 via-black/40 to-black/10" />
        <div className="absolute right-4 top-4">
          {t.status === "REGISTRATION_OPEN" || t.status === "PUBLISHED" ? <Button size="sm" onClick={() => setRegisterOpen(true)}>Register</Button> : <Badge tone={s.tone}>{s.label}</Badge>}
        </div>
        <div className="absolute inset-x-0 bottom-0 p-5">
          <div className="flex items-center gap-2">
            {isLive && <Badge tone="danger">LIVE BROADCAST</Badge>}
            <span className="text-xs font-medium text-white/80">{races.length} race{races.length === 1 ? "" : "s"}</span>
          </div>
          <h2 className="mt-1 text-2xl font-bold text-white">{t.name}</h2>
          {t.description && <p className="mt-1 max-w-xl text-sm text-white/70 line-clamp-2">{t.description}</p>}
        </div>
      </div>
    ...
    /* Race schedule */
          ...
          <div>
            <p className="mb-3 text-xs font-semibold uppercase tracking-wide text-muted">Race Schedule</p>
            {racesQuery.isPending ? <div className="flex flex-col gap-2">{Array.from({ length: 3 }).map((_, i) => <Skeleton key={i} className="h-10 w-full rounded" />)}</div> : races.length === 0 ? <p className="text-sm text-muted">No races scheduled.</p> : <ul className="flex flex-col gap-3">
                {races.slice(0, 6).map((r) => {
    const running = r.status === "RUNNING" || r.status === "OPEN";
    return <li key={r.raceId} className="flex gap-3">
                      <span className={cn("mt-1.5 h-2 w-2 shrink-0 rounded-full", running ? "bg-brand-700" : "bg-border")} />
                      <div className="min-w-0">
                        <p className="text-sm font-medium text-ink">{fmtTime(r.scheduledStartAt)}</p>
                        <p className="truncate text-sm text-ink">{r.name}</p>
                        <Badge tone={RACE_STATUS_TONE[r.status] ?? "neutral"}>{r.status}</Badge>
                      </div>
                    </li>;
  })}
              </ul>}
    ...
    /* Elite entries */
          ...
          <div>
            <div className="mb-3 flex items-center justify-between">
              <p className="text-xs font-semibold uppercase tracking-wide text-muted">Elite Entries</p>
            ...
            {entriesQuery.isPending ? <div className="flex flex-col gap-2">{Array.from({ length: 3 }).map((_, i) => <Skeleton key={i} className="h-12 w-full rounded-lg" />)}</div> : entries.length === 0 ? <p className="text-sm text-muted">No entrants yet.</p> : <ul className="flex flex-col gap-2">
                {entries.slice(0, 4).map((e) => <li key={e.registrationId} className="flex items-center gap-2 rounded-lg border border-border px-3 py-2">
    ...
    /* Entry performance (entries per race — real) */
        ...
        {races.length > 0 && <div className="rounded-xl bg-subtle/40 p-4">
            ...
            <div className="flex h-28 items-end gap-2">
              {races.slice(0, 10).map((r) => <div key={r.raceId} className="flex flex-1 flex-col items-center gap-1" title={`${r.name}: ${r.entriesCount ?? 0}`}>
...
export {
  TournamentCatalogPage as default
};
