...
const schema = z.object({
  fullName: z.string().min(1, "Nh\u1EADp h\u1ECD t\xEAn"),
  phone: z.string().regex(/^$|^\+?[0-9\-\s]{7,30}$/, "S\u1ED1 \u0111i\u1EC7n tho\u1EA1i kh\xF4ng h\u1EE3p l\u1EC7").optional().default("")
});
function ProfilePage() {
  ...
  return <div className="mx-auto max-w-2xl">
      <PageHeader title="Hồ sơ cá nhân" subtitle="Cập nhật thông tin tài khoản của bạn." />
      <Card>
  ...
  />
              <Button size="sm" variant="secondary" loading={avatar.isPending} onClick={onUpload}>
                Tải ảnh lên
              </Button>
            </div>
          </div>
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col gap-4">
            <Input label="Họ và tên" {...register("fullName")} error={errors.fullName?.message} />
            <Input label="Số điện thoại" {...register("phone")} error={errors.phone?.message} />
            <Input label="Email" value={user?.email ?? ""} disabled readOnly />
            <Input label="Vai trò" value={user ? ROLE_LABELS[user.role] : ""} disabled readOnly />
            <Button type="submit" size="lg" loading={update.isPending}>Lưu thay đổi</Button>
          </form>
        </CardBody>
      </Card>
    </div>;
