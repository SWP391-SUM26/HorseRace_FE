...
import { useSubmitPrediction, useClaimReward, useLiveRace, useEntryNames } from './hooks';
...
describe('spectator mutation hooks', () => {
  ...
  it('useSubmitPrediction invalidates the predictions query on success', async () => {
    ...
    result.current.mutate({ raceId: 'r1', predictionType: 'WIN', stakeAmount: 100, predictedEntryId: 'e1' });
    await waitFor(() => expect(result.current.isSuccess).toBe(true));
    expect(spy).toHaveBeenCalledWith({ queryKey: ['spectator', 'predictions'] });
  });
...
describe('useEntryNames — entryId → horseName resolution', () => {
  ...
  afterEach(() => getMock.mockImplementation(async () => ({ data: { data: [] } })));

  it('folds runners across unique races into one Map and de-dupes raceIds', async () => {
    getMock.mockImplementation(async (url: string) => {
      if (url === '/races/r1/entries') return { data: { data: [{ entryId: 'e1', horseName: 'Thunderbolt' }] } };
      if (url === '/races/r2/entries') return { data: { data: [{ entryId: 'e2', horseName: 'Silver Streak' }] } };
      return { data: { data: [] } };
    });
    const { wrapper } = makeWrapper();
    const { result } = renderHook(() => useEntryNames(['r1', 'r1', 'r2']), { wrapper });
