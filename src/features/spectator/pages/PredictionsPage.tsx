...
import { Badge, Button, Card, CardBody, DataTable, EmptyState, Select, Skeleton, Tabs, type Column } from '@/common/ui';
...
import type { PredictionResponse, PredictionType } from '../types';
import type { RaceEntry } from '../api';

// Per-entry Win/Place/Show buttons — same single source as the panel picker.
const SUB_TYPES: PredictionType[] = PREDICTION_TYPE_OPTIONS.map((o) => o.value);
...
export default function PredictionsPage() {
  ...
  // Only OPEN/SCHEDULED races are predictable.
  ...
  const [predictionType, setPredictionType] = useState<PredictionType>('WIN');
  ...
  function pick(entry: RaceEntry, type: PredictionType) {
        ...
        <div className="grid gap-6 lg:grid-cols-[2fr_1fr]">
          <div className="space-y-5">
            {racesQuery.isLoading ? (
              ...
              <EmptyState title="No open races" description="There are no races currently open for predictions." />
            ) : (
              <>
                {/* Race header card */}
                <Card>
                ...
                <Card>
                  <CardBody>
                    ...
                    {entriesQuery.isLoading ? (
                      ...
                      <EmptyState title="No runners" description="This race has no confirmed entries yet." />
                    ) : (
                          ...
                          return (
                            <li
                              ...
                              className={`rounded-xl border p-3 transition-colors ${
                                active ? 'border-brand-600 bg-brand-50' : 'border-border bg-surface'
                              }`}
                            >
                                  ...
                                  return (
                                    <button
                                      ...
                                      className={`h-8 flex-1 rounded-lg text-xs font-semibold transition-colors disabled:cursor-not-allowed disabled:opacity-50 ${
                                        on ? 'bg-brand-700 text-white' : 'bg-subtle text-ink hover:bg-border'
                                      }`}
                                    >
                                      {t}
                                    </button>
          ...
          <div className="lg:sticky lg:top-4 lg:self-start">
            <BettingPanel
              ...
              selectionLabel={selectedEntry ? `${selectedEntry.horseName ?? 'Runner'} · ${predictionType}` : null}
