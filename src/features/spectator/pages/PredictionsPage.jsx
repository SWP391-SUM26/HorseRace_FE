...
import { BettingPanel, MIN_STAKE } from "../components/BettingPanel";
import {
  canPredict,
...
// Per-entry Win/Place/Show buttons — same single source as the panel picker.
...
export default function PredictionsPage() {
  ...
  // Live ESTIMATED pari-mutuel odds for the selected runner in the selected bet-type pool.
  const poolsQuery = useLivePoolOdds(activeRaceId);
  const selectedPool =
    (poolsQuery.data ?? []).find((p) => p.predictionType === predictionType) ??
    null;
  const selectedRunnerOdds = selectedEntryId
    ? (selectedPool?.runners.find((r) => r.entryId === selectedEntryId) ?? null)
    : null;
  ...
  function confirm() {
    ...
    submit.mutate(
      ...
      },
      {
        onSuccess: (p) =>
          toast.success(
            `Prediction confirmed — locked odds ${p.lockedOdds ?? "—"}, potential payout ${
              p.potentialPayout?.toLocaleString("vi-VN") ?? "—"
            }`,
          ),
      },
    );
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        subtitle="Study the field, pick your runner, and place a prediction into an open pool."
      />
        ...
        <div className="grid gap-6 lg:grid-cols-[2fr_1fr]">
          <div className="space-y-5">
            {racesQuery.isLoading ? (
              ...
              <EmptyState
                title="No open races"
                description="There are no races currently open for predictions."
              />
            ) : (
              <>
                {/* Race header card */}
                  ...
                  <CardBody className="space-y-3">
                    {openRaces.length > 1 && (
                      <Select
                        ...
                        options={openRaces.map((r) => ({
                          value: r.raceId,
                          label: r.name ?? r.raceCode ?? r.raceId,
                        }))}
                        value={activeRaceId ?? ""}
                        onChange={(e) => {
                    ...
                    {selectedRace && (
                      <div className="flex flex-wrap items-start justify-between gap-3">
                        <div>
                          <Badge tone="info">
                            {selectedRace.raceType ?? "Feature"}
                          </Badge>
                          <h2 className="mt-2 text-xl font-semibold text-ink">
                            {selectedRace.name ?? selectedRace.raceCode}
                          </h2>
                          <p className="text-sm text-muted">
                            {selectedRace.venueName ??
                              selectedRace.venue ??
                              "—"}{" "}
                            •{" "}
                            {selectedRace.distanceMeter
                              ? `${selectedRace.distanceMeter}m`
                              : "—"}
                          </p>
                          <p className="mt-1 text-sm text-muted">
                            Post: {fmtTime(selectedRace.scheduledStartAt)}
                          </p>
                        </div>
                        <span className="flex items-center gap-1.5 text-sm font-semibold text-danger">
                          <Clock className="h-4 w-4" /> Pool{" "}
                          {selectedRace.status}
                        </span>
                      </div>
                    )}
                ...
                <Card>
                  <CardBody>
                    ...
                    {entriesQuery.isLoading ? (
                      ...
                      <EmptyState
                        title="No runners"
                        description="This race has no confirmed entries yet."
                      />
                    ) : (
                                ...
                                {SUB_TYPES.map((t) => {
                                  ...
                                  return (
                                    <button
                                      ...
                                      disabled={!open}
                                      onClick={() => pick(e, t)}
                                      className={`h-8 flex-1 rounded-lg text-xs font-semibold transition-colors disabled:cursor-not-allowed disabled:opacity-50 ${
                                        on
                                          ? "bg-brand-700 text-white"
                                          : "bg-subtle text-ink hover:bg-border"
                                      }`}
                                    >
                                      {t}
                                    </button>
                  ...
                  <CardBody className="flex items-start gap-3">
                    ...
                    <div>
                      ...
                      <p className="mt-1 text-sm text-ink">
                        Analyst commentary is a preview placeholder — no live
                        tipping feed is wired yet. Study the odds and recent
                        form above before staking.
                      </p>
                    </div>
                  </CardBody>
                </Card>
          ...
          <div className="lg:sticky lg:top-4 lg:self-start">
            <BettingPanel
              ...
              estimatedPayoutPerUnit={
                selectedRunnerOdds?.estimatedPayoutPerUnit ?? null
              }
              poolTotalStake={selectedPool?.totalLiveStake ?? null}
              oddsLoading={poolsQuery.isLoading}
            />
          </div>
        </div>
      ) : (
  ...
  const columns = [
    {
      ...
      render: (p) => (
        <span className="font-medium text-ink">
          {p.raceName ?? p.raceCode ?? p.raceId}
        </span>
      ),
    },
    { key: "type", header: "Type", render: (p) => p.predictionType },
    {
      key: "stake",
      header: "Stake",
      render: (p) => p.stakeAmount.toLocaleString("vi-VN"),
    },
    ...
    },
    {
      ...
      header: "Status",
      render: (p) => (
        <Badge tone={PREDICTION_STATUS_META[p.status].tone}>
          {PREDICTION_STATUS_META[p.status].label}
        </Badge>
      ),
    },
    {
