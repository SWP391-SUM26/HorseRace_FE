...
import { Badge, Card, CardBody, DataTable, EmptyState, Skeleton } from "@/common/ui";
...
function RewardsPage() {
  ...
  const balance = useMemo(() => {
    const winnings = predictions.filter((p) => p.status === "WON").reduce((s, p) => s + (p.potentialPayout ?? 0), 0);
    const claimed = history.filter((r) => r.status === "CLAIMED").reduce((s, r) => s + r.amount, 0);
    return winnings + claimed;
  }, [predictions, history]);
  const tierProgress = Math.min(100, Math.round(balance % TIER_STEP / TIER_STEP * 100));
  const alerts = useMemo(() => {
    const settled = predictions.filter((p) => p.status === "WON" || p.status === "LOST").slice(0, 3).map((p) => ({
      id: p.predictionId,
      text: p.status === "WON" ? `You won on ${p.raceName ?? p.raceCode ?? "a race"} (+${(p.potentialPayout ?? 0).toLocaleString("vi-VN")})` : `No luck on ${p.raceName ?? p.raceCode ?? "a race"}`,
      tone: p.status === "WON" ? "success" : "danger"
    }));
  ...
  const historyColumns = [
    ...
    },
    {
      key: "selection",
      header: "Selection",
      render: (p) => p.predictedEntryId ? entryNames.get(p.predictedEntryId) ?? `Entry ${p.predictedEntryId.slice(0, 6)}` : "\u2014"
    },
    ...
    },
    {
      key: "points",
      header: "Points",
      render: (p) => p.status === "WON" ? `+${(p.potentialPayout ?? 0).toLocaleString("vi-VN")}` : p.status === "LOST" ? `-${p.stakeAmount.toLocaleString("vi-VN")}` : "\u2014"
    }
  ];
  function onClaim(rewardId) {
  ...
  return <div className="space-y-6">
    ...
    /* Available balance — DERIVED (no BE balance endpoint) */
        ...
        <div className="rounded-2xl bg-brand-900 p-6 text-white">
          ...
          {predictionsQuery.isLoading || historyQuery.isLoading ? <Skeleton className="mt-3 h-10 w-40" /> : <div className="mt-2 flex items-center gap-3">
    ...
    /* Recent alerts — DERIVED */
        ...
        <Card>
          <CardBody>
            ...
            {predictionsQuery.isLoading ? <Skeleton className="h-24 w-full" /> : alerts.length ? <ul className="space-y-2">
                {alerts.map((a) => <li key={a.id} className="flex items-center gap-2 rounded-lg bg-bg px-3 py-2">
                    <Badge tone={a.tone}>•</Badge>
                    <span className="text-sm text-ink">{a.text}</span>
                  </li>)}
              </ul> : <p className="text-sm text-muted">No recent alerts.</p>}
          </CardBody>
        </Card>
      </div>
  ...
  />
    ...
    /* Trophy Room & Shop */
      ...
      <div>
        ...
        {pendingQuery.isLoading || historyQuery.isLoading ? <div className="grid gap-4 sm:grid-cols-3">
            <Skeleton className="h-40 w-full rounded-2xl" />
            <Skeleton className="h-40 w-full rounded-2xl" />
            <Skeleton className="h-40 w-full rounded-2xl" />
          </div> : shopRewards.length ? <div className="grid gap-4 sm:grid-cols-3">
            {shopRewards.map((r) => <RewardRow key={r.rewardId} reward={r} onClaim={onClaim} claiming={claim.isPending} />)}
          </div> : <EmptyState title="No rewards yet" description="Earn rewards by predicting and reaching milestones." />}
      </div>
    </div>;
}
export {
