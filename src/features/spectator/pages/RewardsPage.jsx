...
import { PREDICTION_STATUS_META } from "../constants";
...
export default function RewardsPage() {
  ...
  const entryNames = useEntryNames(
    useMemo(() => predictions.map((p) => p.raceId), [predictions]),
  );
  ...
  const tierProgress = Math.min(
    100,
    Math.round(((balance % TIER_STEP) / TIER_STEP) * 100),
  );
  ...
  const historyColumns = [
    {
      key: "date",
      header: "Date",
      render: (p) => fmtDate(p.submittedAt ?? p.createdAt ?? null),
    },
    {
      key: "event",
      header: "Event & Race",
    ...
    },
    {
      ...
      render: (p) =>
        p.predictedEntryId
          ? (entryNames.get(p.predictedEntryId) ??
            `Entry ${p.predictedEntryId.slice(0, 6)}`)
          : "—",
    },
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        subtitle="Your balance, prediction history, and the trophy room."
      />

      <div className="grid gap-6 lg:grid-cols-[1.3fr_1fr]">
        {/* Available balance — DERIVED (no BE balance endpoint) */}
        <div className="rounded-2xl bg-brand-900 p-6 text-white">
          <p className="text-xs font-semibold uppercase tracking-wide text-brand-50/70">
            Available Balance
          </p>
          ...
          <p className="mt-1 text-xs text-brand-50/60">
            Derived from settled winnings + claimed rewards
          </p>
          <div className="mt-5">
      ...
      <div>
        ...
        {pendingQuery.isLoading || historyQuery.isLoading ? (
          ...
          <EmptyState
            ...
            description="Earn rewards by predicting and reaching milestones."
          />
        )}
      </div>
    </div>
