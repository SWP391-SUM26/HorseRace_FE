...
import { Card, CardBody, DataTable, EmptyState, Skeleton, Tabs, type Column } from '@/common/ui';
...
export default function SpectatorLivePage() {
  ...
  const oddsColumns: Column<RaceEntry>[] = [
  ...
  const resultColumns: Column<RaceResultRow>[] = [
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          {running ? (
            ...
            <span className="text-xs font-bold uppercase tracking-wide text-muted">{race?.status ?? 'Race'}</span>
          )}
      ...
      <div className="grid gap-6 lg:grid-cols-[2fr_1fr]">
        <div className="space-y-4">
          ...
          {liveQuery.isLoading ? (
            ...
            <LiveRunningOrder runners={live?.runningOrder ?? []} running={running} />
          )}
        </div>
      ...
      {!race && !racesQuery.isLoading && (
        <EmptyState title="Race not found" description="This race is no longer available." />
      )}
    </div>
  );
}
