...
describe("PREDICTION_TYPE_OPTIONS — single-runner bet types", () => {
  it("offers exactly WIN/PLACE/SHOW", () => {
    expect(PREDICTION_TYPE_OPTIONS.map((o) => o.value)).toEqual([
      "WIN",
      "PLACE",
      "SHOW",
    ]);
...
describe("canPredict — betting-window gating", () => {
  it("allows predictions only for a CLOSED (locked, pre-cutoff) race", () => {
    expect(canPredict("CLOSED")).toBe(true);
  });
  it("blocks predictions for every non-CLOSED status", () => {
    expect(canPredict("SCHEDULED")).toBe(false);
    expect(canPredict("OPEN")).toBe(false);
