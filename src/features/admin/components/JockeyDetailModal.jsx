...
function RejectReasonModal({ open, onClose, loading, onSubmit }) {
...
/**
 * Jockey detail drawer with approve / reject actions.
 *
 * @param fallback Row data shown immediately while the fresh detail loads.
 */
export function JockeyDetailModal({ fallback, onClose }) {
  ...
  const handleApprove = () => {
    approve.mutate(j.userId, {
      ...
      onError: (err) =>
        toast.error(getApiErrorMessage(err, "Could not approve the jockey.")),
    });
  };
  ...
  const handleReject = (reason) => {
    reject.mutate(
      { id: j.userId, reason },
      {
        ...
        onError: (err) =>
          toast.error(getApiErrorMessage(err, "Could not reject the jockey.")),
      },
    );
  };
  ...
  return (
    ...
    >
      {/* `fallback` (the row data) always seeds initial data, so there is no
          loading state to gate on. */}
      <div className="flex flex-col gap-5">
        <div className="flex items-center gap-3">
          ...
          {j.status && (
            <span className="ml-auto">
              <Badge tone={USER_STATUS_TONE[j.status] ?? "neutral"}>
                {humanize(j.status)}
              </Badge>
            </span>
          )}
        ...
        <div className="grid grid-cols-2 gap-4 sm:grid-cols-3">
          ...
          <Field
            label="Body weight"
            value={j.bodyWeight != null ? `${j.bodyWeight} kg` : null}
          />
          <Field
            label="Height"
            value={j.heightCm != null ? `${j.heightCm} cm` : null}
          />
          <Field
            label="Experience"
            value={
              j.experienceYrs != null
                ? `${j.experienceYrs} yr${j.experienceYrs === 1 ? "" : "s"}`
                : null
            }
          />
        </div>
        ...
        {(j.jockeyLicenseUrl || j.fitnessCertificateUrl) && (
          <div className="rounded-lg border border-border bg-card p-4">
            ...
            <div className="flex flex-wrap gap-2">
              {j.jockeyLicenseUrl && (
                <DocButton label="License document" path={j.jockeyLicenseUrl} />
              )}
              {j.fitnessCertificateUrl && (
                <DocButton
                  label="Fitness certificate"
                  path={j.fitnessCertificateUrl}
                />
              )}
            </div>
          </div>
        ...
        <div className="rounded-lg border border-border bg-card p-4">
          ...
          <div className="grid grid-cols-2 gap-4 sm:grid-cols-4">
            ...
            <Field
              label="Win rate"
              value={j.winRate != null ? `${Math.round(j.winRate * 100)}%` : null}
            />
            <Field
              label="Rating"
              value={j.rating != null ? j.rating.toFixed(1) : null}
            />
            <Field label="Last trophy" value={j.lastTrophy} />
          </div>
          {j.recentForm && j.recentForm.length > 0 && (
            <div className="mt-3">
              <p className="mb-1.5 text-xs uppercase tracking-wide text-muted">
                Recent form
              </p>
              <div className="flex flex-wrap gap-1.5">
                {j.recentForm.map((f, i) => (
                  <span
                    key={i}
