...
export function UserDetailModal({ user, onClose }) {
  ...
  return (
    <Modal open onClose={onClose} size="lg" title="User Detail">
      <div className="flex flex-col gap-5">
        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          <div className="min-w-0">
            ...
            <div className="mt-1 flex flex-wrap items-center gap-2">
              ...
              {user.status && (
                <Badge
                  tone={USER_STATUS_TONE[user.status.toUpperCase()] ?? "neutral"}
                >
                  {user.status}
                </Badge>
              )}
              ...
              {horses.map((h) => (
                ...
                >
                  ...
                  <div className="p-2">
                    ...
                    <p className="truncate text-xs text-muted">{h.breed ?? "—"}{h.status ? ` · ${h.status}` : ""}</p>
                  </div>
                </div>
              ))}
            </div>
        ...
        <div>
          ...
          {winsQuery.isPending ? (
            ...
            <p className="text-sm text-muted">Couldn't load wins.</p>
          ) : wins.length === 0 ? (
            <p className="text-sm text-muted">No race wins yet.</p>
          ) : (
              ...
              {wins.map((w, i) => (
                ...
                >
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <p className="truncate text-xs text-muted">{w.tournamentName ?? "—"}{w.scheduledStartAt ? ` · ${formatDate(w.scheduledStartAt)}` : ""}</p>
                  </div>
                  {w.prizeEarned != null && w.prizeEarned > 0 && (
                    <span className="shrink-0 text-sm font-medium text-brand-700">
                      ${w.prizeEarned.toLocaleString()}
                    </span>
                  )}
                </li>
              ))}
