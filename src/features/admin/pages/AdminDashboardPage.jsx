...
/** One KPI tile that resolves its own loading/error state independently. */
function Kpi({ label, icon, pending, error, value, hint }) {
  if (pending) {
    return (
      <Card className="p-6">
...
export default function AdminDashboardPage() {
  ...
  const sections = [
    {
      label: "Tournaments",
      description: "Create, publish and orchestrate tournaments",
      to: "/admin/tournaments",
      icon: <Trophy size={18} />,
    },
    {
      label: "Race Calendar",
      description: "Schedule and manage races",
      to: "/admin/races",
      icon: <Flag size={18} />,
    },
    {
    ...
    },
    {
      ...
      description: "Review horse race registrations",
      to: "/admin/race-approval",
      icon: <ClipboardList size={18} />,
    },
    {
      label: "Withdrawals",
      description: "Review spectator withdrawal requests",
  ...
  return (
    <div className="flex flex-col gap-6">
      <PageHeader
        ...
        subtitle="Cross-domain overview of the racing operation."
      />
        ...
        <Kpi
          ...
          hint={
            <span className="text-muted">
              {raceStats.data?.scheduled ?? 0} scheduled ·{" "}
              {raceStats.data?.active ?? 0} active
            </span>
          }
        />
        <Kpi
          ...
          hint={
            <span className="text-warning">
              {regStats.data?.pending ?? 0} pending review
            </span>
          }
        />
        <Kpi
          ...
          hint={
            <span className="text-muted">
              {staffing.data?.availableReferees ?? 0} referees available
            </span>
          }
        />
        <Kpi
