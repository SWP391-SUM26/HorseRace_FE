...
export default function StaffingDetailPage() {
  ...
  return (
    <>
      ...
      ) : raceQuery.isError || !race ? (
        <Card>
          <CardBody>
            <EmptyState
              title="Couldn't load race"
              description="Go back to the staffing list."
            />
          </CardBody>
        </Card>
      ) : (
            ...
            <div className="min-w-0">
              ...
              <p className="mt-1 text-sm text-muted">
                ...
                {race.scheduledStartAt
                  ? ` · ${formatDate(race.scheduledStartAt)}`
                  : ""}
              </p>
            </div>
            <Button onClick={() => setAdding(true)}>
              <UserPlus size={16} /> Assign Referees
            </Button>
          </CardBody>
        </Card>
      )}
            ...
            <ul className="divide-y divide-border">
              {panel.map((a) => (
                ...
                >
                  ...
                  {a.panelRole ? (
                    <Badge tone={ROLE_TONE[a.panelRole] ?? "neutral"}>
      ...
      {adding && race && (
        <AssignPanelModal raceId={raceId} onClose={() => setAdding(false)} />
      )}
    </>
  );
}
