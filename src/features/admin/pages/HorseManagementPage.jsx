...
import { HORSE_GENDER_FILTERS, HORSE_HEALTH_TONE, HORSE_STATUS_FILTERS, HORSE_STATUS_TONE } from "../constants";
function HorseManagementPage() {
  ...
  const injuredCount = rows.filter((h) => h.healthStatus && h.healthStatus !== "HEALTHY").length;
  return <>
      <PageHeader
  ...
  />
    ...
    setPage(0);
        ...
        <div className="w-full sm:w-44">
          <Select label="Gender" value={gender} onChange={(e) => {
    ...
    setPage(0);
      ...
      <Card>
        <CardBody className="p-0">
          {listQuery.isPending ? <div className="flex flex-col gap-2 p-4">{Array.from({ length: 5 }).map((_, i) => <Skeleton key={i} className="h-12 w-full rounded-lg" />)}</div> : listQuery.isError ? <EmptyState title="Couldn't load horses" description="Please reload the page." /> : rows.length === 0 ? <EmptyState title="No horses" description="No horses match the current filters." /> : <div className="overflow-x-auto">
              <table className="w-full min-w-[760px] text-sm">
                ...
                <tbody className="divide-y divide-border">
                  {rows.map((h) => <tr key={h.horseId} className="cursor-pointer hover:bg-subtle/40" onClick={() => setOpenHorse(h)}>
                      <td className="px-4 py-3">
                        <p className="font-medium text-ink">{h.name ?? h.horseCode}</p>
                        <p className="text-xs text-muted">{h.horseCode}</p>
                      </td>
                      <td className="px-4 py-3 text-muted">{h.ownerName ?? "\u2014"}</td>
                      <td className="px-4 py-3 text-muted">
                        {h.gender ? humanize(h.gender) : "\u2014"}
                        <span className="text-xs text-muted"> · {h.breed ?? "\u2014"}</span>
                      </td>
                      <td className="px-4 py-3">
                        {h.healthStatus ? <Badge tone={HORSE_HEALTH_TONE[h.healthStatus] ?? "neutral"}>{humanize(h.healthStatus)}</Badge> : "\u2014"}
                      </td>
                      <td className="px-4 py-3">
                        {h.status ? <Badge tone={HORSE_STATUS_TONE[h.status] ?? "neutral"}>{humanize(h.status)}</Badge> : "\u2014"}
                      </td>
    ...
    setOpenHorse(h);
      ...
      {openHorse && <HorseDetailModal horse={openHorse} onClose={() => setOpenHorse(null)} />}
    </>;
}
export {
  HorseManagementPage as default
};
