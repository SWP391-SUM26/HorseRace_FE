...
export default function WithdrawalsPage() {
  ...
  function onReject(id) {
    reject.mutate(id, {
      onSuccess: () =>
        toast.success("Withdrawal rejected — hold returned to the wallet."),
    });
  }
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (w) => (
        <span className="font-medium text-ink">{formatMoney(w.amount)}</span>
      ),
    },
    {
      key: "requested",
      header: "Requested",
      render: (w) => <span className="text-muted">{fmtTime(w.createdAt)}</span>,
    },
    {
      key: "status",
      header: "Status",
      render: (w) => (
        <Badge tone={STATUS_TONE[String(w.status)] ?? "neutral"}>
          {String(w.status)}
        </Badge>
      ),
    },
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        title="Withdrawals"
        subtitle="Review and action spectator withdrawal requests."
      />
      <Card>
