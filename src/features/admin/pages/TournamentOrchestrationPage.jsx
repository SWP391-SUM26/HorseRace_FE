...
/** Maps a tournament status to the badge label + tone shown in the catalog. */
...
export default function TournamentOrchestrationPage() {
  ...
  const listQuery = useTournaments({ status: status || undefined });
  const tournaments = listQuery.data?.rows ?? [];

  const selected = useMemo(() => {
    if (tournaments.length === 0) return null;
    return (
      tournaments.find((t) => t.tournamentId === selectedId) ?? tournaments[0]
    );
    ...
    <>
      ...
      <div className="grid gap-6 lg:grid-cols-5">
        ...
        <div className="lg:col-span-3">
          {selected ? (
            <DetailPanel key={selected.tournamentId} tournament={selected} />
          ) : (
            <Card>
              <CardBody>
                <EmptyState
                  title="Select a tournament"
                  description="Pick an event to manage it."
                />
              </CardBody>
            </Card>
          )}
    ...
    >
      ...
      <div className="p-3">
        ...
        <p className="mt-1 flex items-center gap-1 text-xs text-muted">
          ...
          {t.startDate ? formatDate(t.startDate) : "—"} · {t.tournamentCode}
        </p>
...
function DetailPanel({ tournament }) {
  ...
  const entriesQuery = useRegistrations({
    tournamentId: tournament.tournamentId,
    size: 100,
  });
  ...
  const races = useMemo(
    () =>
      [...(racesQuery.data?.rows ?? [])].sort((a, b) =>
        (a.scheduledStartAt ?? "").localeCompare(b.scheduledStartAt ?? ""),
      ),
    [racesQuery.data],
  );
  const entries = (entriesQuery.data?.rows ?? []).filter(
    (r) =>
      r.status !== "WITHDRAWN" &&
      r.status !== "REJECTED" &&
      r.status !== "REMOVED",
  ...
  const s = statusDisplay(t.status);
  const entriesCount =
    t.registeredEntriesCount ?? t.registeredEntries ?? entries.length;
  ...
  return (
    <Card className="overflow-hidden">
      ...
      <CardBody className="flex flex-col gap-6">
        ...
        <div className="grid grid-cols-2 gap-4 sm:grid-cols-4">
          ...
          <Stat
            label="Entries"
            value={`${entriesCount}${
              t.entryCap != null ? ` / ${t.entryCap}` : ""
            }`}
          />
          <Stat
            label="Dates"
            value={`${t.startDate ? formatDate(t.startDate) : "—"}${
        ...
        <div className="grid grid-cols-1 gap-6 sm:grid-cols-2">
          ...
          <div>
            ...
            {racesQuery.isPending ? (
              ...
              <p className="text-sm text-muted">No races yet.</p>
            ) : (
                ...
                {races.slice(0, 6).map((r) => (
                  <li key={r.raceId}>
                    <button
                      ...
                      className="flex w-full items-center gap-3 rounded-lg border border-border px-3 py-2 text-left hover:bg-subtle/50"
                    >
                      <span
                        className={cn(
                          "h-2 w-2 shrink-0 rounded-full",
                          r.status === "RUNNING" || r.status === "OPEN"
                            ? "bg-brand-700"
                            : "bg-border",
                        )}
                      />
            ...
            {entriesQuery.isPending ? (
              ...
              <ul className="flex flex-col gap-2">
                {entries.slice(0, 5).map((e) => (
                  <li key={e.registrationId}>
                    <button
                      type="button"
                      onClick={() => setOpenReg(e)}
                      className="flex w-full items-center gap-2 rounded-lg border border-border px-3 py-2 text-left hover:bg-subtle/50"
                    >
                      <div className="min-w-0 flex-1">
                        <p className="truncate text-sm font-medium text-ink">
                          {e.horseName ?? "—"}
                      ...
                      <Badge tone={REGISTRATION_STATUS_TONE[e.status]}>
            ...
            <div className="flex h-24 items-end gap-2">
              {races.slice(0, 10).map((r) => (
                <div
                  key={r.raceId}
                  className="flex-1"
                  title={`${r.name ?? r.raceCode}: ${r.entriesCount ?? 0}`}
                >
                  <div
                    ...
                    style={{
                      height: `${Math.max(
                        6,
                        ((r.entriesCount ?? 0) / maxEntries) * 100,
                      )}%`,
                    }}
                  />
                </div>
      ...
      {addingRace && (
        <RaceFormModal
          mode="create"
          tournaments={[{ tournamentId: t.tournamentId, name: t.name }]}
          lockedTournamentId={t.tournamentId}
          onClose={() => setAddingRace(false)}
        />
      )}
      {openRace && (
        <RaceDetailModal
          raceId={openRace.raceId}
          fallback={openRace}
          tournaments={[{ tournamentId: t.tournamentId, name: t.name }]}
