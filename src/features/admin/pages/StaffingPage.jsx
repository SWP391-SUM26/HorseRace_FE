    ...
    <>
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
        {dash.isPending || !d ? (
          ...
          <>
            <StatCard
              label="Total Scheduled Races"
              value={d.totalScheduledRaces}
              icon={<CalendarDays size={18} />}
            />
            <StatCard
              label="Races With Panel"
              value={d.assignedReferees}
            ...
            <StatCard
              ...
              hint={
                d.unassignedRaces > 0 ? (
                  ...
                  <span className="text-muted">All races staffed</span>
                )
              }
            />
            <StatCard
              label="Available Referees"
              value={d.availableReferees}
              icon={<UserPlus size={18} />}
              hint={
                <span className="text-muted">Available for assignment</span>
              }
            />
          </>
        )}
      ...
      <div className="mt-4 mb-4">
        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          <Button variant="secondary" className="shrink-0" onClick={clearFilters}>
            Clear Filters
          </Button>
        </div>
      </div>
              ...
              <table className="w-full min-w-[720px] text-sm">
                ...
                <tbody className="divide-y divide-border">
                  {rows.map((a) => {
                    const count =
                      a.assignmentCount ??
                      (a.assignmentStatus === "ASSIGNED" ? 1 : 0);
                    return (
                      <tr
                        key={a.raceId}
                        className="cursor-pointer hover:bg-subtle/40"
                        onClick={() => open(a.raceId)}
                      >
                          ...
                          <div className="flex items-center gap-3">
                            ...
                            <div>
                              <p className="font-medium text-ink">
                                {a.raceName ?? a.raceCode ?? "—"}
                              </p>
                              <p className="text-xs text-muted">
                                ID: {a.raceCode ?? a.raceId}
                              </p>
                            </div>
                          </div>
                        </td>
      ...
      <div className="mt-4 flex flex-wrap items-center justify-between gap-3">
        <p className="text-sm text-muted">
          {total > 0 ? `Showing ${pageFrom} to ${pageTo} of ${total} races` : ""}
        </p>
        {totalPages > 1 && (
          <div className="flex flex-wrap items-center gap-1">
            ...
            {totalPages > 5 && (
              <Button
                size="sm"
                variant={page === totalPages - 1 ? "primary" : "ghost"}
      ...
      {assigningRaceId && (
        <AssignPanelModal
          raceId={assigningRaceId}
          onClose={() => setAssigningRaceId(null)}
        />
      )}
    </>
  );
