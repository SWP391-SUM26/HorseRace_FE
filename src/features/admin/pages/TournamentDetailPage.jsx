  ...
  return (
    <>
        ...
        <Card>
          <CardBody>
            <EmptyState
              ...
              description="It may have been removed. Go back to the list."
            />
          </CardBody>
        </Card>
      ) : (
        <div className="relative overflow-hidden rounded-2xl bg-gradient-to-br from-green-950 to-green-700">
          <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-black/30 to-transparent" />
          <div className="relative flex flex-wrap items-center gap-2 p-5">
            <Badge tone={TOURNAMENT_STATUS_TONE[t.status]}>
          ...
          <div className="relative p-5 pt-3">
            <div className="flex flex-wrap items-start justify-between gap-3">
              <div className="min-w-0">
                ...
                {t.location && (
                  <p className="mt-0.5 text-sm text-white/60">{t.location}</p>
                )}
                {t.description && (
            ...
            <div className="mt-4 grid grid-cols-2 gap-4 border-t border-white/20 pt-4 sm:flex sm:flex-wrap sm:gap-8">
              <div>
                ...
                <p className="text-lg font-semibold text-white">
                  {t.totalPurse != null
                    ? `${Math.round(t.totalPurse).toLocaleString("vi-VN")}₫`
                    : "—"}
                </p>
              </div>
              <div>
      ...
      <div className="mt-6 flex flex-wrap items-center justify-between gap-3">
        ...
        <Button size="sm" disabled={!t} onClick={() => setAddingRace(true)}>
      ...
      <Card className="mt-3">
        <CardBody className="p-0">
          {racesQuery.isPending ? (
            ...
            <EmptyState
              title="No races yet"
              description="Add the first race for this tournament."
            />
          ) : (
                  ...
                  {races.map((r) => (
                    ...
                    >
                      ...
                      <td className="px-4 py-3 text-xs text-muted whitespace-nowrap">
                        {r.scheduledStartAt
                          ? formatDate(r.scheduledStartAt)
                          : "—"}
                      </td>
                      <td className="px-4 py-3 text-muted">
                        {r.entriesCount ?? 0} / {r.maxParticipants ?? "—"}
                      </td>
                      <td className="px-4 py-3">
                        <Badge tone={RACE_STATUS_TONE[r.status]}>
