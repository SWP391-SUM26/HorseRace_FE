...
import { REGISTRATION_STATUS_FILTERS, REGISTRATION_STATUS_TONE } from "../constants";
...
function RegistrationApprovalPage() {
  ...
  return <>
    ...
    actions={<Button variant="secondary" onClick={exportCsv}><FileText size={16} /> Export CSV</Button>}
  />
      ...
      <div className="mt-4 mb-4 flex flex-wrap items-end gap-3">
        <div className="w-full flex-1 sm:min-w-56">
          <Input label="Search" value={q} onChange={(e) => {
    setQ(e.target.value);
    setPage(0);
    ...
    setPage(0);
      ...
      <Card>
        <CardBody className="p-0">
          {listQuery.isPending ? <div className="flex flex-col gap-2 p-4">{Array.from({ length: 5 }).map((_, i) => <Skeleton key={i} className="h-12 w-full rounded-lg" />)}</div> : listQuery.isError ? <EmptyState title="Couldn't load registrations" description="Please reload the page." /> : rows.length === 0 ? <EmptyState title="No registrations" description="Nothing to review here." /> : <div className="overflow-x-auto">
              <table className="w-full min-w-[680px] text-sm">
                ...
                <tbody className="divide-y divide-border">
                  {rows.map((r) => <tr key={r.registrationId} className="cursor-pointer hover:bg-subtle/40" onClick={() => setViewing(r)}>
                      <td className="px-4 py-3">
                        <p className="font-medium text-ink">{r.horseName ?? "\u2014"}</p>
                        <p className="text-xs text-muted">{r.ownerName ?? "\u2014"} · {r.registrationCode}</p>
                      </td>
                      <td className="px-4 py-3 text-muted">{r.tournamentName ?? "\u2014"}</td>
                      <td className="px-4 py-3 text-xs text-muted">{r.submittedAt ? formatDate(r.submittedAt) : "\u2014"}</td>
                      <td className="px-4 py-3"><Badge tone={REGISTRATION_STATUS_TONE[r.status]}>{r.status.replace("_", " ")}</Badge></td>
    ...
    setViewing(r);
      ...
      {viewing && <RegistrationDetailModal registration={viewing} onClose={() => setViewing(null)} />}
    </>;
}
export {
  RegistrationApprovalPage as default
};
