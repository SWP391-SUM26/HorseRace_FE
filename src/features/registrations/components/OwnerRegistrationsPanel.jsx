...
/** Owner's tournament registrations — embedded as a tab in the Tournament Catalog. */
export function OwnerRegistrationsPanel() {
  const { user } = useAuth();
  ...
  if (rows.length === 0) {
    return (
      <EmptyState
        ...
        description="Register a horse from the Catalog tab to track its approval status here."
      />
    );
  }
        ...
        <div className="flex items-start gap-2 rounded-lg bg-red-50 px-3 py-2">
          ...
          <div>
            ...
            <p className="text-sm text-ink">
              {reg.rejectionReason || "No reason provided."}
            </p>
          </div>
        </div>
      )}

      {canWithdraw(reg.status) && (
